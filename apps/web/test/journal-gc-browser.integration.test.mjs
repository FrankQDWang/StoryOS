...
import { collectEligibleJournalPayload } from "/apps/web/src/journal-payload-collection.mjs";
...
const SNAPSHOT = "018f0000-0000-7001-8000-000000000022";
...
const session = {
  ...
  base_snapshot: { snapshot_id: SNAPSHOT, chapter_id: CHAPTER,
...
const fetchImpl = async (url, options = {}) => {
  ...
  if (/draft/i.test(path)) fail("journal collection created a Recovery Draft route: " + path);
...
try {
  ...
  deepEqual(await collectEligibleJournalPayload(workspace), { fences: [] },
    "unsettled payload is not eligible");
  deepEqual(await readJournalSnapshot(workspace), unsettled,
    "unsettled journal payload remains retained");
  const saved = await submitOnePendingAuthorEdit({
  ...
  }, "applied settlement before collection");
  ...
  deepEqual(collection, { fences: [expectedFence] }, "collection writes one inspectable fence");
  const collected = await readJournalSnapshot(workspace);
  deepEqual(collected.fences, [expectedFence], "durable fences remain inspectable");
  ...
  equal(collected.payloadChains[0].ordered_patch_refs[0].normalized_primitives, undefined,
    "patch payload bytes are collected");
  deepEqual(collected.groups[0].frozen_request_body.author_edit_units, [],
    "frozen request payload bytes are collected");
  ...
  equal(collected.groups[0].journal_submission_group_id, group.journal_submission_group_id,
    "group identity remains");
  equal(collected.groups[0].settlement.kind, "applied_receipt_settled",
    "settlement evidence remains");
  deepEqual(await rebuildPendingProjection(workspace), {
    body: "Base!?", save_state: "saved", unsettled_intent_count: 0,
    authoritative_revision_id: NEXT_REVISION,
  }, "successor projection remains after collection");
  await persistReplaceSelection(workspace, {
  ...
  deepEqual(await collectEligibleJournalPayload(workspace), { fences: [expectedFence] },
    "a later unsettled record is not collected");
  const retained = await readJournalSnapshot(workspace);
  equal(retained.records.length, 2, "collected identity and new unsettled record remain");
  equal(retained.records[1].author_edit_unit.normalized_primitives[0].text, "+",
    "only-complete-copy unsettled payload remains retained");
  deepEqual(retained.fences, pending.fences, "later collect does not rewrite the fence");
  workspace.database.close();
  await requestResult(indexedDB.deleteDatabase(journalName));
  sessionStorage.removeItem("active_session:" + OWNER + ":" + PROJECT);
  await report("pass", "journal payload collection passed");
  document.body.dataset.result = "pass";
  document.body.textContent = "journal payload collection passed";
} catch (error) {
...
test("journal payload collection requires settlement, successor, and no remaining dependency", {
  ...
  const allowed = new Set([
    ...
    "/apps/web/src/journal-payload-collection.mjs",
  ...
  const profileDirectory = mkdtempSync(join(tmpdir(), "storyos-journal-gc-browser-"));
  ...
  try {
    const result = await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(
        () => reject(new Error("real-browser journal payload collection timed out")), 20_000,
      ).unref()),
    ]);
    assert.deepEqual(result, {
      result: "pass",
      text: "journal payload collection passed",
    });
  } finally {
    server.close();
