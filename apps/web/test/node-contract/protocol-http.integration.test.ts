...
async function startRealServer(): Promise<{
  ...
  return new Promise((resolve, reject) => {
    ...
    let stdout = "", stderr = "";
    const fail = (error: Error) => { clearTimeout(timeout); server.kill("SIGTERM"); reject(error); };
    ...
    server.stdout.on("data", (chunk) => {
      ...
      if (baseUrl !== undefined) { clearTimeout(timeout); resolve({ baseUrl, server }); }
    });
  });
}

async function stopRealServer(server: ChildProcess): Promise<void> {
