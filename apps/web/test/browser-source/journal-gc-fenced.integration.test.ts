...
it("collects a fenced partition under its immutable old writer generation", async () => {
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    if (path.includes("/manuscript/author-edit-outcomes/")) {
      ...
      const outcome: GetApplyAuthorEditOutcomeResponse = {
  ...
  try {
    ...
    expect(workspace.partition).toEqual({
      ...openPartition,
      disposition: "read_only_observer",
    });
