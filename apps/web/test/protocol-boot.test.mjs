...
const compatible = () => fixture("get-protocol-profile.json");
const messages = {
...
const blocked = (code, details) => ({ kind: "protocol-blocked", code,
...
const blockedCases = [
  ["a missing compatibility identity fails closed with an inspectable message", () => [
    fixture("get-protocol-profile.invalid.json"),
  ...
  ["a stale generated-client identity requires an upgrade before protected state", () => [
    fixture("get-protocol-profile.boundary.json"),
  ...
  ].map(([name, target, field]) => [name, () => {
    const profile = compatible();
    (target ? profile[target] : profile)[field] = "storyos.unknown.v1";
