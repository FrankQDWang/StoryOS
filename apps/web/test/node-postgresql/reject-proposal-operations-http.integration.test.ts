// Verification: {"phase":"http-main","after":["apps/web/test/node-postgresql/settle-multi-operation-selections-http.integration.test.ts"]}
...
test("rejectProposalOperations freezes one pending Operation without changing authority", async () => {
  ...
  try {
    ...
    const prepared = await prepare(started.baseUrl, id("f3800111"), "Reject Proposal Novel", "f3802");
    ...
    const queried = await admitProse(started.baseUrl, prepared.fetchImpl, prepared.projectId, prepared.chapterId, id("f3800131"));
    ...
    const wrongAdmission: RejectProposalOperationsRequest = {
      ...
      reject_proposal_operations_input: {
        ...
        editor_session_id: id("f3800160"),
        correlation_id: id("f3800161"),
      },
    };
