...
function createChallengeRequest(idempotencyKey: string, title: string): CreateProjectChallengeRequest {
  return {
    ...
    create_project_input: {
      ...
      correlation_id: "018f0000-0000-7001-8000-00000000c10",
    },
...
test("exportHumanReadableManuscript admits one inspectable in-progress operation", async () => {
  ...
  try {
    const first = await createEmpty(baseUrl, "session-a", "018f0000-0000-7001-8000-00000000c01", "Empty Novel");
    const request = exportRequest("018f0000-0000-7001-8000-00000000c11");
    const applied = await postExport(
      baseUrl,
      first.fetchImpl,
      first.projectId,
      "018f0000-0000-7001-8000-00000000c21",
      request,
    ).catch((error) => {
    ...
    const replay = await exportHumanReadableManuscript({
      ...
      projectId: first.projectId,
      fetchImpl: first.fetchImpl,
      idempotencyKey: "018f0000-0000-7001-8000-00000000c21",
    ...
    await assert.rejects(
      getHumanReadableManuscriptExport({
        baseUrl,
        projectId: first.projectId,
        exportId: "018f0000-0000-7001-8000-00000000c99",
        fetchImpl: first.fetchImpl,
      }),
      (error) => requireStoryOSProtocolError(error).status === 404,
    );

    const foreign = await createEmpty(baseUrl, "session-b", "018f0000-0000-7001-8000-00000000c02", "Other Novel");
    await assert.rejects(
      getHumanReadableManuscriptExport({
        baseUrl,
        projectId: first.projectId,
    ...
    await assert.rejects(
      postExport(
        ...
        foreign.fetchImpl,
        first.projectId,
        "018f0000-0000-7001-8000-00000000c23",
        exportRequest("018f0000-0000-7001-8000-00000000c13"),
      ),
      (error) => {
        const protocol = requireStoryOSProtocolError(error);
        return protocol.status === 404 && !String(protocol.responseBody).includes(USER_A);
    ...
    const historical = await postExport(
      ...
      first.fetchImpl,
      first.projectId,
      "018f0000-0000-7001-8000-00000000c28",
      exportRequest("018f0000-0000-7001-8000-00000000c18"),
    );
    ...
    const archiveDigest = await digestArchiveProject(archiveRequest("1", "018f0000-0000-7001-8000-00000000c16"));
    const archiveChallenge = await withChallengeRetry(() => createProjectCommandChallenge({
      ...
      request: {
        ...
        idempotency_key: "018f0000-0000-7001-8000-00000000c26",
      },
    }));
    const archived = await archiveProject({
      ...
      projectId: first.projectId,
      fetchImpl: first.fetchImpl,
      idempotencyKey: "018f0000-0000-7001-8000-00000000c26",
      antiForgery: archiveChallenge.nonce,
      request: archiveRequest("1", "018f0000-0000-7001-8000-00000000c16"),
    });
    assert.equal(archived.effect.kind, "authoritative_applied");

    const refusedRequest = exportRequest("018f0000-0000-7001-8000-00000000c17");
    ...
    const refusedChallenge = await withChallengeRetry(() => createProjectCommandChallenge({
      ...
      request: {
        ...
        idempotency_key: "018f0000-0000-7001-8000-00000000c27",
      },
    }));
    await assert.rejects(
      exportHumanReadableManuscript({
        ...
        projectId: first.projectId,
        fetchImpl: first.fetchImpl,
        idempotencyKey: "018f0000-0000-7001-8000-00000000c27",
