...
type InFlightCreateChapter = {
  idempotencyKey: string;
  correlationId: string;
  nonce?: string;
};

const inFlightCreates = new Map<string, InFlightCreateChapter>();

function createChapterIdentity(options: {
  projectId: string;
  volumeId: string;
  title: string;
...
export async function createOwnedChapter(options: {
  baseUrl: string;
...
async function submitCreateChapter(
  ...
  if (flight.nonce === undefined) {
    const challenge = await createProjectCommandChallenge({
      ...
      request: {
        ...
        canonical_command_digest: await digestCreateChapter(request, options.cryptoImpl),
  ...
  if (nonce === undefined) {
    throw new Error("the Create Chapter challenge nonce is missing");
  }
