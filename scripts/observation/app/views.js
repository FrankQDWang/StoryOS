...
function reviewView(findings) {
    ...
    return '<p class="muted">来自现有 violations 规则视图。被拒绝的请求不算执行；心跳过期表示存活未知。</p><div class="toolbar"><select aria-label="规则类别" data-review-setting="rule">'+selectOptions([['all','全部规则'],...Object.entries(ruleNames)],findings.rule)+'</select><select aria-label="执行事实" data-review-setting="disposition">'+selectOptions([['all','全部事实'],['executed','实际启动'],['prevented','请求已阻止'],['unknown-liveness','存活未知']],findings.disposition)+'</select><select aria-label="规则排序" data-review-setting="sort">'+selectOptions([['newest','最新优先'],['oldest','最早优先']],findings.sort)+'</select></div>'+rules.map(rule=>{
        const items=findings.rows.filter(row=>row.rule===rule&&(findings.disposition==='all'||row.disposition===findings.disposition)).sort((a,b)=>(findings.sort==='oldest'?1:-1)*((parseTime(a.started_at)||0)-(parseTime(b.started_at)||0)));
...
function historyView(model, selected) {
    return '<div class="toolbar"><input aria-label="搜索运行" placeholder="搜索任务、类型或运行 ID" data-setting="q" value="'+escapeText(model.q)+'"><select aria-label="运行状态" data-setting="status">'+selectOptions([['','全部状态'],['passed','通过'],['failed','失败'],['running','未完成（含心跳过期）'],['interrupted','中断']],model.status)+'</select><select aria-label="执行事实" data-setting="activity">'+selectOptions([['','全部运行'],['started','实际启动'],['reused','结果复用'],['not-started','未启动']],model.activity)+
