...
function costView(model) {
    ...
    const rootNote=root?'':'<p class="notice">此记录没有实际根启动或成本事实。被拒绝与复用不产生新根费用。</p>';
    ...
    return rootNote+'<p class="notice">成本是独占墙钟时间；父子阶段不相加。并发区间只计一次，未归类区间单列。</p>'+(root?'<dl><dt>本轮根耗时</dt><dd>'+duration(root.seconds)+'</dd><dt>本轮阻塞等待</dt><dd>'+duration(root.blocked_seconds)+' · '+(root.blocked_seconds==null?'未测量':'显式测量')+'</dd></dl><h3>本轮独占阶段</h3>'+stages(bill.stages):'')+
...
function graphView(model,run) {
    ...
    const nodes=graph.nodes.filter(node=>(node.type!=='test-file'||model.graphFilter==='all')&&
    ...
    return '<p class="notice">依赖边表示执行前提；包含和成员边仅表示归属。选择不等于实际执行。</p><div class="scope"><span>节点 '+graph.nodes.length+'</span><span>依赖 '+graph.dependencies.length+'</span><span>实际启动节点 '+attempted.size+'</span></div>'+analysisLinks(run)+
        '<h3>步骤与节点</h3><div class="toolbar wrap"><input aria-label="搜索图节点" placeholder="搜索步骤或节点 ID" data-graph-search value="'+escapeText(model.graphSearch)+'"><select aria-label="图节点范围" data-graph-filter>'+selectOptions([['active','已选择或已启动'],['selected','已选择'],['attempted','已启动'],['all','全部节点']],model.graphFilter)+'</select></div><p class="muted">'+nodes.length+' 个节点 · 选择节点查看前提、成员与尝试</p>'+nodes.map(node=>{
...
function comparisonView(model,run,right) {
    ...
    const differenceNames={'common':'共同定义','only-left':'仅左侧','only-right':'仅右侧','changed-definition':'定义变化',unavailable:'范围未知'};
    const quality=data.comparison.comparison, differences=data.differences.filter(row=>model.compareFilter==='all'||model.compareFilter==='changed'&&(row.visibleChanged??(row.difference!=='common'||row.left_selected!==row.right_selected||row.left_attempts!==row.right_attempts||row.left_state!==row.right_state))||(row.displayDifference||row.difference)===model.compareFilter);
    differences.sort((a,b)=>model.compareSort==='id'?a.node_id.localeCompare(b.node_id):(a.displayDifference||a.difference).localeCompare(b.displayDifference||b.difference)||a.node_id.localeCompare(b.node_id));
