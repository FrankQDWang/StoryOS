...
US41 adds a collapsed needs-review list for every existing rule. The run summary
opens cost, graph and timeline, and two-run comparison without a diagnostics step.
The graph starts with selected or attempted steps; the comparison starts with
changed nodes. A filter can reveal all retained nodes. Each deeper view has a
contextual Back action. The existing full DAG and comparison dashboards open in
new tabs. Their links have an underline, hover state, focus ring, and new-tab cue.
The source list keeps its filter, order, scroll, and open rule groups on return.
