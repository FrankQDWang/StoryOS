...
class RustCacheTests(unittest.TestCase):
    ...
    def test_default_debug_is_outside_managed_cache(self):
        debug = self.root / "target/debug"
        debug.mkdir()
        (debug / "user-data").write_text("keep")
        result = self.cli("status")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual((debug / "user-data").read_text(), "keep")
        self.assertFalse((self.root / "target/verification/legacy-rust-cache.json").exists())
