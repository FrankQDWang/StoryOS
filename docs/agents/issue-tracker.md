...
- **Verification order:** open the PR, then wait for the required GitHub `verify` check, the automated pull-request review comments, and the independent read-only Standards and Spec reviews. Fix every finding with a targeted check, then push the corrected range once. Then run `make verify-local` once and record its evidence. A later commit on the PR range invalidates that evidence, so a verification that starts before the reviews end must run again. A red targeted check is not a reason to start `make verify-local`.
