...
- **Verification order:** open the PR, then wait for the GitHub `verify` sentinel, automated review comments, and independent read-only Standards and Spec reviews. Fix findings with targeted checks and push the corrected range. Run `make verify-local` once, publish with `make verify-evidence`, then wait for `candidate-evidence`. A changed tree or base invalidates the report. A red targeted check is fixed with another targeted check. The initial evidence-gate installation uses the existing gates, then validates publication and activates the new requirement on protected main.
- **Completion:** merge the ticket PR into `main` and synchronize `main`. When the merged tree SHA equals the tree SHA in the PR evidence comment, run `make verify-tracker` only, record the equality, and link that comment. When it differs, run `make verify`. Record the exact merge commit, tree, command, and result in the ticket Resolution, refresh the current map, and close the ticket.
