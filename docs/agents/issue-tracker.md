...
- **Verification order:** open the PR, then wait for the required GitHub `verify` check, the automated pull-request review comments, and the independent read-only Standards and Spec reviews. Fix every finding, then push the corrected range once. Then run the complete local verification once and record its evidence. A later commit on the PR range invalidates that evidence, so a verification that starts before the reviews end must run again.
- **Completion:** merge the ticket PR into `main` and synchronize `main`. When the merged tree SHA equals the tree SHA in the PR evidence comment, run `make verify-tracker` only, record the equality, and link that comment. When it differs, run complete verification. Record the exact merge commit, tree, command, and result in the ticket Resolution, refresh the current map, and close the ticket.
