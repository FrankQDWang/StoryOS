...
- **Verification order:** open the PR, then wait for the GitHub `verify` sentinel, automated review comments, and independent read-only Standards and Spec reviews. Fix findings with targeted checks and push the corrected range. A red targeted check is fixed with another targeted check. Run `make verify-local` only when the ticket contract or an explicit request requires a complete local run. The retired `candidate-evidence` gate and report publication are not merge requirements.
- **Completion:** merge the ticket PR into `main` and synchronize `main`. Run `make verify-tracker`. Record the exact merge commit, tree, command, and result in the ticket Resolution, refresh the current map, and close the ticket.
