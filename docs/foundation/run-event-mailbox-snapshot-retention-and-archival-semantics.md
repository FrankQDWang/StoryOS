...
Artifact, Context Assembly Manifest, Destination Context Manifest,
Outbound Disclosure Manifest, and PostgreSQL contracts. It does not authorize
a schema, Rust, client, deployment, or storage implementation.
...
4. every dependency applicable to the payload, including Admission, Receipt,
   ...
   Manifest, Destination Context Manifest, Outbound Disclosure Manifest for an
   External Processing Destination, Proposal or Draft lifecycle transition, and
...
| Physical Deletion Completion | For every StoryOS-controlled online, archive, and Recovery Copy of the erased payload, the authorized retention window has expired or the copy has been verifiably cleaned. An unexpired Recovery Copy without verified cleanup keeps the lifecycle in Physical cleanup pending; this does not assert deletion from an external destination or an already delivered Project Export. | Completion identity, scope of completed cleanup, remaining Tombstone/manifest/gap/provenance floor, and the settled external-effect uncertainty. |
