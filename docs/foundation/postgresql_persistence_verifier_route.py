...
def validate_route_coverage(catalog: dict[str, Any], route_catalog: dict[str, Any], family_map: dict[str, dict[str, Any]], operation_owners: dict[str, set[str]], event_owners: dict[str, set[str]], errors: list[str]) -> dict[str, int]:
    ...
    for operation in operations:
        ...
        if operation_id in required:
            ...
            for event_id in activity_event_ids:
                ...
                elif event_id in {"storyos.event.editor-flow-draft-closed.v1", "storyos.event.editor-flow-draft-reopened.v1"}:
    ...
    for event_id in required_events:
        if event_id in {"storyos.event.refused-edit-draft-created.v1", "storyos.event.editor-flow-draft-closed.v1", "storyos.event.editor-flow-draft-reopened.v1"}:
            if "artifact-proposal-draft" not in event_owners.get(event_id, set()) or event_by_id[event_id].get("wire_profile") != "storyos.artifact-lifecycle.v1":
