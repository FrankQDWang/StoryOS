...
- Exact release baseline consumed: `main@3c2e4009b9af69d851c126daf6b4fc429a0393ca`
- Exact baseline tree consumed: `9317f27407f5abe199423a2ada5001c90d7a0399`
- Predecessor release-contract revision: `product-delivery-realignment-2026-08-28-v1`
...
`HND-*`, and the Stage 1 through Stage 10 meanings. The owner contracts listed in section 4
...
| `B-EVAL` | Read-only Eval View query, evidence availability/redaction/gap facts, explicit assessment boundary, and no-effect-on-open/no-authority result; a separately admitted Assessment records its normal Attempt and disclosure facts. |
...
| `DVG-12` Standalone Eval boundary | `OWN-EVAL` + `OWN-CTX` + `OWN-RET`; proves Eval View/read/refresh is read-only, explicit assessment follows normal boundaries, evidence gaps/redaction remain visible, and Eval cannot write prose, route authority, or cause implicit model execution or egress. A separately admitted explicit Assessment uses the normal Context and Attempt boundary. | `FX-EVAL-READONLY`, `FX-ABSENT-EXECUTION` · `CFP-MANIFEST-BEFORE-COMMIT`, `CFP-DISPATCH-BEFORE-CLAIM`, `CFP-LIFECYCLE-AFTER-INVALIDATION-BEFORE-CLEANUP` · `SCH-ABSENT`, `SCH-LIFECYCLE`, `SCH-UNKNOWN` · `ORC-EVAL-READONLY`. | `B-EVAL`; `PASS-POS` for the read-only view or admitted Assessment facts; `PASS-REFUSAL` for execution excluded by the selected stage or denied by its owner. `BLOCK-ALL`; Eval/advisory output cannot upgrade a stage. |
...
| `ORC-EVAL-READONLY` | Opening or refreshing Eval reads evidence only; no model, egress, authority, or hidden routing occurs. The companion's positive Assessment oracle is separate. |
...
Stages 5 through 10 deliver governed Tools/research, Skills, Memory, MCP Apps,
complete orchestration, and advisory Eval. Their positive proof selections and
...
`SMAP-STAGE-1` through `SMAP-STAGE-10` are mandatory implementation-evidence
maps: each selects its registered row here or in the companion and never contains `EV-SR`.
`SMAP-EVALUATED-MANDATORY-STAGE` selects exactly one of those ten maps from
...
The separate `SMAP-RELEASE-STAGE-1` through `SMAP-RELEASE-STAGE-10` selectors
...
of those ten release rows, keyed by the same `evaluated_stage` that selects
...
| `REL-004` | The first four stages retain their identities and are followed by the accepted Stages 5 through 10; `OWN-REL`, `OWN-STAGE`. | `DVG-01`, `DVG-02`, `DVG-03`, `DVG-04`, `DVG-05`, `DVG-06`, `DVG-07`, `DVG-08`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-12`, `DVG-13` | `EC-01`; `EV-CP/SE` | `FX-HANDOFF`, `FX-CONTRACT-R1` · contract drift and stage-boundary points · `SCH-DRIFT/NORMAL/ABSENT` · `ORC-CROSSWALK-COMPLETENESS` · `CFP-CONTRACT-DRIFT` | `B-HANDOFF` + `B-CONTRACT`; `PASS-STAGE` when the accepted ten-stage sequence and evidence dependency graph match [Define the AI-Independent Editor-First Release Baseline and Handoff Criteria](https://github.com/FrankQDWang/StoryOS/issues/62). | `BLOCK-ALL`; any unapproved stage, reordered stage, invented capability, or unresolved owner/gate row blocks. |
...
- every accepted `REL-*`, `HND-*`, and Stage 1 through Stage 10 Requirement,
