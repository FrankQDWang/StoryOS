...
| `B-S4-MANDATORY-SET` | Exact Stage 4 mandatory bundle set, formed from the complete Stage 2 set plus one real-destination advisory boundary; members are `B-CONTRACT`, `B-SCOPE`, `B-EDITOR`, `B-CORE`, `B-RECOVERY`, `B-REPLAY`, `B-RESTORE`, `B-CONTEXT`, `B-REAL-ADVISORY`, `B-ABSENT`, `B-MEASURE`, and `B-HANDOFF`. It contains no fake-model bundle and does not make real-destination evidence a substitute for Stage 2. |
...
| `B-S4-REQ-006-AUTHOR-JOURNEY-SET` | Exact Stage 4 author-journey set; members are `B-CONTRACT`, `B-SCOPE`, `B-EDITOR`, `B-CORE`, `B-RECOVERY`, `B-REPLAY`, `B-RESTORE`, `B-CONTEXT`, `B-REAL-ADVISORY`, `B-ABSENT`, `B-MEASURE`, and `B-HANDOFF`. The real route therefore carries the complete Stage 2 editor evidence. |
| `B-S4-JRN-001-SET` | Exact Stage 4 journey set; members are `B-CONTRACT`, `B-SCOPE`, `B-EDITOR`, `B-CORE`, `B-RECOVERY`, `B-REPLAY`, `B-RESTORE`, `B-CONTEXT`, `B-REAL-ADVISORY`, `B-ABSENT`, `B-MEASURE`, and `B-HANDOFF`. The model-disabled regression is separately selected as `B-S2-MANDATORY-SET`. |
...
| `REL-005` | The currently evaluated stage is eligible only when its own author journey and mandatory evidence are current, attributable, replayable, and passed; the evidence record supplies exactly one `evaluated_stage`; `OWN-REL`, `OWN-DVG`. Resolve `SMAP-EVALUATED-STAGE` once for that stage. A later stage's unrun evidence is not a current-stage failure. | `SMAP-EVALUATED-STAGE` | `SMAP-EVALUATED-STAGE` | `SMAP-EVALUATED-STAGE` | `SMAP-EVALUATED-STAGE`; `PASS-STAGE` only after the selected stage's journey and mandatory statuses are passed or have the row-specific expected negative/hold disposition. | `BLOCK-ALL`; a selected-stage mandatory item that is failed, unrun, stale, unavailable, unverified, or unreplayable blocks, while evidence belonging only to a later stage does not. |
...
| `S3-JRN-001` | Six-step Stage 3 journey—current passage/request, fake full path, edit/accept/reject, refusal/conflict/interruption, and fake-off repeat of the complete Stage 2 journey—is executable; `OWN-REL` with `OWN-PLAIN`, `OWN-CTX`, `OWN-AGENT`, `OWN-CORE`, `OWN-WEB`. `SMAP-STAGE-2` is consumed in full before the fake-specific additions. | `SMAP-STAGE-2` + `DVG-04`, `DVG-05`, `DVG-06` | `SMAP-STAGE-2` + `EC-08` | `SMAP-STAGE-2` + `FX-FAKE-MODEL` | `B-S2-MANDATORY-SET` + `B-FAKE`; `PASS-STAGE`. | `BLOCK-ALL`; any fake-path step, proposal decision, unknown recovery fact, or complete Stage 2 journey/mandatory item is missing, failed, stale, unrun, unavailable, unverified, or unreplayable. |
...
| `S4-JRN-001` | Seven-step Stage 4 journey—standalone editor, bounded request, pre-egress manifest, edit/accept/reject, post-dispatch unknown, reconciliation/recovery, and model-disabled repeat of the complete Stage 2 journey—is executable; `OWN-REL` and all Stage 4 owners. `SMAP-STAGE-2` is consumed in full; real-destination evidence remains an additional advisory path. | `SMAP-STAGE-2` + `DVG-04`, `DVG-05` | `SMAP-STAGE-2` + `EC-08`; `EV-RDA` | `SMAP-STAGE-2` + `FX-REAL-MODEL-ADVISORY` | `B-S2-MANDATORY-SET` + `B-REAL-ADVISORY`; `PASS-STAGE`. | `BLOCK-ALL`; any real-route boundary, manifest, unknown recovery, author decision, or complete Stage 2 journey/mandatory item is missing, failed, stale, unrun, unavailable, unverified, or unreplayable. |
