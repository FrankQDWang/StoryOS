...
10. **No hidden authority.** Tool, MCP, App, Provider, Eval, cache, model
    ...
    by the author.
11. **Positive domain classification.** Admission issuance, sanitized
    pre-admission refusal, OutcomeUnknown, reconciliation, terminal settlement,
    Editor Input Fence, Author Action, and typed Receipt evidence remain
    Operational Records; Refused Edit and Recovery Drafts remain Draft
    Artifacts; Proposal Conflict remains a Proposal validation-axis condition;
    and only the named Core transition may append an Authoritative Revision.
    Every Artifact Creator and Receipt producer cause is one exact closed
    variant, with no browser, model, MCP, extension, local-projection, journal,
    or generic `System` fallback.
12. **Protected-client claim ceiling.** A positive author-command case proves
    only that the exact admitted Protected Web Client build, Client Session
    Binding generation, accepted client-contract and security-policy
    identities, server-derived User and Project Scope, applicable Editor
    Session and writer generation, action class, digest, nonce/idempotency
    identities, and lifetime matched. No browser event, synthetic fixture,
    local journal, cache, projection, or test harness may upgrade that result
    into proof of one physical human gesture, trusted display, user presence,
...
| `DVG-11` Security negative-evidence corpus | Execute the trust model's hostile Origin, XSS/DOM sink, dependency or asset substitution, third-party script, browser-extension interference, stale-build/service-worker cache, stale-tab/takeover, bridge, Tool/MCP, prompt, Provider, SSRF, archive, role/RLS, credential, log/support/telemetry, controlled-cloud boundary-classification, stale-worker, replay, tamper, restore, and resource-bound cases. For each, inspect emitted public output and all safe test artifacts rather than only source code or an HTTP status. | Negative Evidence Closure: no unauthorized authority, context, egress, Attempt, budget effect, secret, or undeclared foreign identity; non-oracular public shape; safe scanned artifacts. A leak, accepted mismatched client/security identity, false human-attestation claim, or forbidden side effect is `failed`. | Trust model; Protocol; PostgreSQL; Context/Disclosure; Retention. |
...
- XSS and dangerous DOM sinks, dependency or immutable-asset substitution,
  ambient third-party script, browser-extension interference, stale
  service-worker/application caches, and stale-tab or writer-takeover
  submissions across exact Protected Web Client, client-contract,
  security-policy, Client Session, Editor Session, and writer generations;
...
- tampered, duplicate, delayed, reordered, stale, crashed, replayed, or
  restore-derived durable facts and resource-bound exhaustion; and
- controlled-cloud classifications that attempt to infer StoryOS control,
  Project Isolation, or disclosure eligibility from hostname, network
  location, infrastructure ownership, Provider brand, or shared operator
  account rather than exact registered control and scope evidence.
