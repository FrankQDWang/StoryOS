...
- Status: current Release 1 verification contract for Wayfinder #60
...
- Contract revision: `deterministic-gates-editor-first-2026-07-31-issue60`
- Exact release baseline consumed: `main@b9aa0d3a718441eebe6affb551e9b9b8eb56078f`
- Exact baseline tree consumed: `e41946962e2700c247e0bddca81d51e26f176114`
...
- change the four stages, author journeys, product priority, required or absent
  capabilities, controlled-cloud position, or handoff order in #62;
...
- modify Issue #1, #60, #62, #77, the Wayfinder map, or `.reference/**`.
...
produced. Stage evidence and stage release consume the applicable gate bundles;
they do not certify themselves. A physical restore drill must inspect the
restored service and lifecycle facts independently of the backup job's own
success flag.
...
| `OWN-AGENT` | The one general Project-scoped Agent Loop and AgentRun owner named by [Wayfinder #1](https://github.com/FrankQDWang/StoryOS/issues/1). This key does not create a task-specific runtime or add a new Agent contract. |
| `OWN-MODEL` | The Model Gateway/Provider owner named by [Wayfinder #1](https://github.com/FrankQDWang/StoryOS/issues/1). Provider internals and model-quality claims remain outside deterministic proof. |
| `OWN-TOOL` | The Tool owner named by [Wayfinder #1](https://github.com/FrankQDWang/StoryOS/issues/1). Tool execution is explicitly absent from Stages 3 and 4. |
| `OWN-MCP` | The MCP/MCP App owner named by [Wayfinder #1](https://github.com/FrankQDWang/StoryOS/issues/1). MCP execution is explicitly absent from Stages 3 and 4. |
...
| `OWN-MEASURE` | A named owner-adopted measurement contract, including [#76](https://github.com/FrankQDWang/StoryOS/issues/76) where applicable. No measurement row may invent a target value. |
...
| `CFP-EDITOR-AFTER-SETTLEMENT-BEFORE-ACK` | Durable settlement exists before browser acknowledgement/Activity; replay must converge without duplication. |
...
| --- | --- |
| `SCH-NORMAL` | Bind exact scope/session → admit → perform owner transition → commit durable facts → emit Receipt/Event → acknowledge → replay and compare. |
| `SCH-REORDER` | Commit durable facts → deliver Event before ack → deliver ack before Event → duplicate both → resync from Snapshot; compare one settled effect and one activity history. |
...
| --- | --- | --- | --- |
| `DVG-01` Contract source, generated schema/catalog, golden wire, same-release identity, bounds, and drift | `OWN-PROTO` + `OWN-PG` + `OWN-GOV`; proves source-to-generated-to-wire/catalog parity, exact release identity, accepted limits, and fail-closed drift. | `FX-CONTRACT-R1` · `CFP-CONTRACT-BEFORE-GENERATION`, `CFP-CONTRACT-AFTER-GENERATION-BEFORE-WIRE`, `CFP-CONTRACT-DRIFT` · `SCH-DRIFT` · `ORC-CONTRACT`. | `B-CONTRACT`; `PASS-POS` for parity or `PASS-REFUSAL`/`PASS-HOLD` for drift. `BLOCK-ALL`. |
| `DVG-02` Request/query/cursor/Project Scope isolation | `OWN-PROTO` + `OWN-ADM` + `OWN-PG` + `OWN-TRUST`; proves exact User/Project Scope, Host/Origin/session generations, requester derivation, bounds, cursor handling, forced RLS, and non-oracular refusal. | `FX-SCOPE-2U2P` · `CFP-SCOPE-BEFORE-QUERY`, `CFP-REPLAY-BELOW-FLOOR`, `CFP-RESTORE-STAGING` · `SCH-SCOPE`, `SCH-REPLAY` · `ORC-SCOPE`. | `B-SCOPE`; `PASS-POS` for valid scope, `PASS-REFUSAL` for substitution, or `PASS-HOLD` for invalid recovery visibility. `BLOCK-ALL`. |
| `DVG-03` Author edit, Web Editor Session, Proposal, Acceptance/Rejection, Receipt, Author Action, and undo transaction | `OWN-WEB` + `OWN-ADM` + `OWN-CORE` + `OWN-PLAIN`; proves browser input becomes only the owner-defined journal/group/command, Core outcomes are atomic, Proposal decisions are explicit, and no agent/chat/retry fabricates authority. | `FX-EDITOR-IME`, `FX-CORE-PROPOSAL` · `CFP-EDITOR-BEFORE-JOURNAL-DURABILITY`, `CFP-EDITOR-AFTER-ADMISSION-BEFORE-CORE`, `CFP-CORE-BEFORE-COMMIT`, `CFP-CORE-AFTER-COMMIT-BEFORE-ACK`, `CFP-PROPOSAL-BEFORE-DECISION`, `CFP-PROPOSAL-AFTER-ACCEPTANCE-BEFORE-RECEIPT`, `CFP-UNDO-BEFORE-SETTLEMENT` · `SCH-NORMAL`, `SCH-CRASH`, `SCH-REORDER` · `ORC-ATOMIC-AUTHORITY`. | `B-EDITOR` + `B-CORE`; `PASS-POS`, `PASS-REFUSAL`, or `PASS-HOLD` according to the scheduled result. `BLOCK-ALL`. |
...
3. **Atomic visible settlement:** a Core transition is absent or has its
   complete write set; refusal/conflict may have its required no-change Receipt
   but never a partial authority effect.
...
| `ORC-ATOMIC-AUTHORITY` | A Core write set is all-or-nothing; typed Receipt/Author Action/Proposal result matches durable state and exact idempotency. |
...
The four stages remain exactly those accepted by #62. The proof order is:
...
| `REL-004` | Exactly four stages and their accepted order remain unchanged; `OWN-REL`/`OWN-STAGE`. | `DVG-01`, `DVG-13` plus all applicable stage gates | `EC-01`; `EV-CP/SE/SR` | `FX-HANDOFF`, `FX-CONTRACT-R1` · contract drift and stage-boundary points · `SCH-DRIFT/NORMAL/ABSENT` · `ORC-CROSSWALK-COMPLETENESS` · `CFP-CONTRACT-DRIFT` | `B-HANDOFF` + `B-CONTRACT`; `PASS-STAGE` when the four-stage sequence and evidence dependency graph match #62. | `BLOCK-ALL`; any new stage, reordered stage, invented capability, or unresolved owner/gate row blocks. |
| `REL-005` | A stage is eligible only when its author journey and mandatory evidence are current, attributable, replayable, and passed; `OWN-REL`/`OWN-DVG`. | `DVG-01`, `DVG-02`, `DVG-03`, `DVG-04`, `DVG-05`, `DVG-06`, `DVG-07`, `DVG-08`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-12`, `DVG-13` plus the row-specific gate result | `EC-01/02/03/04/05/06/07/08`; `EV-CP/IT/INT/PRD/RDA/SE/SR` | `FX-HANDOFF` plus the stage fixture · `CFP-CONTRACT-DRIFT` · `SCH-DRIFT`, `SCH-NORMAL`, and the applicable recovery/negative schedules · `ORC-CROSSWALK-COMPLETENESS` | `B-HANDOFF` plus every row bundle; `PASS-STAGE` only after all mandatory statuses are `Passed` or the row-specific expected negative/hold disposition. | `BLOCK-ALL`; failed/unrun/stale/unavailable/unverified/unreplayable evidence blocks even if another stage or advisory check is green. |
| `REL-006` | Planning, implementation, stage evidence, stage release, and later controlled-cloud deployment remain separate; `OWN-REL`/`OWN-GOV`. | `DVG-01`, `DVG-10`, `DVG-13` | `EC-01/04/05`; `EV-CP/PRD/SE/SR/CCD` | `FX-HANDOFF`, `FX-RESTORE-LIFECYCLE` · contract drift, restore, deletion, deployment-identity points · `SCH-DRIFT/RESTORE/LIFECYCLE` · `ORC-CONTRACT`, `ORC-RESTORE-LIFECYCLE`, `ORC-CROSSWALK-COMPLETENESS` · `CFP-CONTRACT-DRIFT` | `B-HANDOFF` + `B-RESTORE`; `PASS-STAGE` for local separation or `PASS-CLOUD` only for later controlled-cloud evidence. | `BLOCK-ALL`; planning cannot be reported as implementation/release, and cloud evidence cannot backfill a local gap. |
...
| `S1-REQ-003` | Direct edit goes through Admission/Core to authoritative revision, typed Receipt, Author Action/Activity, and save state; `OWN-ADM`, `OWN-CORE`, `OWN-PROTO`, `OWN-PG`. | `DVG-02`, `DVG-03`, `DVG-07` | `EC-03/07`; `EV-CP/IT/INT/SE` | `FX-CORE-PROPOSAL`, `FX-JOURNAL-GROUP` · `CFP-ADMISSION-BEFORE-CORE`, `CFP-CORE-BEFORE-COMMIT`, `CFP-CORE-AFTER-COMMIT-BEFORE-ACK`, `CFP-EDITOR-AFTER-SETTLEMENT-BEFORE-ACK` · `SCH-NORMAL/CRASH/REORDER` · `ORC-ATOMIC-AUTHORITY` | `B-CORE` + `B-EDITOR`; `PASS-POS` for applied command, exact `PASS-REFUSAL`/`PASS-HOLD` for other scheduled outcomes. | `BLOCK-ALL`; partial write, wrong Receipt producer, duplicate action, or guessed ack blocks. |
...
| `S1-JRN-001` | The complete six-step controlled writing journey—open, CN/EN/IME input and clipboard, save observation, interrupt/restart settled/unsettled, journal convergence/recovery, one author effect/Receipt/Activity/no duplicate/scope—is executable; `OWN-REL` with `OWN-WEB`, `OWN-CORE`, `OWN-ADM`, `OWN-PG`. | `DVG-02`, `DVG-03`, `DVG-07`, `DVG-08`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-13` | `EC-02/03/04/05/07`; `EV-CP/INT/PRD/SE/SR` | `FX-EDITOR-IME`, `FX-JOURNAL-GROUP`, `FX-RECOVERY-EDITOR` · all editor/core/fence/replay/restore points selected by the journey · `SCH-NORMAL/REORDER/CRASH/FENCE/RESTORE` · `ORC-EDITOR-JOURNAL`, `ORC-RECOVERY-ATOMICITY`, `ORC-SCOPE` · `CFP-EDITOR-BEFORE-JOURNAL-DURABILITY` | `B-EDITOR` + `B-RECOVERY` + `B-HANDOFF`; `PASS-STAGE`. | `BLOCK-ALL`; any missing journey step or unresolved durable fact blocks Stage 1. |
...
| `S1-EVD-003` | Core/Admission/persistence evidence proves atomicity, Receipt, revision, Activity, and idempotency; `OWN-ADM`, `OWN-CORE`, `OWN-PG`. | `DVG-02`, `DVG-03`, `DVG-07` | `EC-03`; `EV-CP/IT/INT/SE` | `FX-CORE-PROPOSAL` · admission/core/outbox/settlement points · `SCH-NORMAL/CRASH/REORDER` · `ORC-ATOMIC-AUTHORITY` · `CFP-CORE-BEFORE-COMMIT` | `B-CORE`; `PASS-POS`. | `BLOCK-ALL`; missing durable fact, duplicate effect, or wrong typed result blocks. |
| `S1-EVD-004` | Reload/restart/recovery-draft/reconfirmation/fence/resync evidence is complete; `OWN-WEB`, `OWN-ADM`, `OWN-CORE`, `OWN-RET`. | `DVG-03`, `DVG-07`, `DVG-08`, `DVG-09`, `DVG-10` | `EC-04/07`; `EV-CP/INT/PRD/SE` | `FX-RECOVERY-EDITOR` · crash/expiry/fence/replay/restore points · `SCH-CRASH/FENCE/REPLAY/RESTORE` · `ORC-RECOVERY-ATOMICITY`, `ORC-OUTCOME-UNKNOWN` · `CFP-CORE-AFTER-COMMIT-BEFORE-ACK` | `B-RECOVERY` + `B-RESTORE`; `PASS-POS` or `PASS-HOLD` by expected recovery state. | `BLOCK-ALL`; any guessed commit, non-replayable recovery, or absent hold blocks. |
| `S1-EVD-005` | Isolation/trust evidence proves scope, Host/Origin/session, forced RLS, and bounded client claims; `OWN-TRUST`, `OWN-PROTO`, `OWN-PG`. | `DVG-02`, `DVG-07`, `DVG-10`, `DVG-11` | `EC-05`; `EV-CP/IT/INT/PRD/SE` | `FX-SCOPE-2U2P`, `FX-RESTORE-LIFECYCLE` · scope/RLS/restore/fence/delete points · `SCH-SCOPE/RESTORE/FENCE/LIFECYCLE` · `ORC-SCOPE`, `ORC-NEGATIVE-CLOSURE` · `CFP-SCOPE-BEFORE-QUERY` | `B-SCOPE` + `B-RESTORE`; `PASS-POS` and `PASS-REFUSAL` as scheduled. | `BLOCK-ALL`; any leak, role bypass, or stale writer effect blocks. |
...
| `S2-REQ-008` | Long session, repeated chapter, reload, controlled upgrade, and measured envelope use only values adopted by the named owner; `OWN-MEASURE`/`OWN-REL` with `OWN-WEB`, `OWN-PG`, `OWN-RET`. | `DVG-01`, `DVG-03`, `DVG-07`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-13` | `EC-01/02/04/05/06`; `EV-IT/INT/PRD/SE/SR` | `FX-LONG-SESSION`, `FX-RECOVERY-EDITOR`, `FX-RESTORE-LIFECYCLE` · owner-adopted measurement/recovery/upgrade points · `SCH-LONG/CRASH/REPLAY/RESTORE` · `ORC-RECOVERY-ATOMICITY`, `ORC-REPLAY-TRUTH` · `CFP-REPLAY-BEFORE-COMPACTION` | `B-MEASURE` + `B-RECOVERY` + `B-RESTORE`; `PASS-STAGE` only for adopted values and complete journey. | `BLOCK-ALL`; unadopted target, stale measurement, or missing recovery/storage provenance blocks. |
| `S2-JRN-001` | Complete twelve-step editor journey: AI-disabled init, hierarchy, CN/EN input, save state, reload/crash/server/PG restart, isolated restore/roles/RLS/projections/lifecycle/visibility/write, takeover, search/replace, stats, replay/resync, export/archive, long session; `OWN-REL` and all Stage 2 owners. | `DVG-01`, `DVG-02`, `DVG-03`, `DVG-07`, `DVG-08`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-13` | `EC-01/02/03/04/05/06/07`; `EV-CP/IT/INT/PRD/SE/SR` | `FX-EDITOR-IME` ·· all applicable editor/core/recovery/replay/restore/lifecycle/scope points · `SCH-NORMAL/CRASH/REORDER/FENCE/REPLAY/RESTORE/LIFECYCLE/LONG` · `ORC-CROSSWALK-COMPLETENESS`, `ORC-RECOVERY-ATOMICITY`, `ORC-RESTORE-LIFECYCLE` · `CFP-SCOPE-BEFORE-QUERY` | All named bundles including `B-HANDOFF`; `PASS-STAGE`. | `BLOCK-ALL`; any of the twelve steps absent, stale, unrun, unavailable, or unreplayable blocks Stage 2. |
...
| `S3-REQ-006` | Bounded inspectable fake result/unknown recovery proves the boundary but makes no model/provider quality claim; `OWN-MODEL`, `OWN-CTX`, `OWN-AGENT`, `OWN-REL`. | `DVG-04`, `DVG-05`, `DVG-06`, `DVG-08`, `DVG-09`, `DVG-11` | `EC-04/08`; `EV-CP/INT/RDA/SE` | `FX-FAKE-MODEL`, `FX-REAL-MODEL-ADVISORY` · dispatch/unknown/reconcile/fence/late points · `SCH-UNKNOWN/FENCE/REPLAY` · `ORC-OUTCOME-UNKNOWN`, `ORC-DISPATCH-DISCLOSURE` · `CFP-DISPATCH-AFTER-IO-BEFORE-CONFIRMATION` | `B-FAKE`; `PASS-POS`/`PASS-UNKNOWN`; `B-REAL-ADVISORY` may be `advisory` only. | `BLOCK-ALL` for missing fake proof; advisory cannot upgrade and Provider-quality claims are invalid. |
| `S3-REQ-007` | When fake assistance is unavailable, S2 remains usable and no fallback capability is invented; `OWN-REL`, `OWN-WEB`, `OWN-CORE`, `OWN-AGENT`. | `DVG-01`, `DVG-03`, `DVG-06`, `DVG-09`, `DVG-11`, `DVG-13` | `EC-01/02/04/05`; `EV-CP/IT/INT/SE` | `FX-FAKE-MODEL`, `FX-ABSENT-EXECUTION`, `FX-RECOVERY-EDITOR` · fake unavailable/fence/lifecycle/absence points · `SCH-ABSENT/CRASH/LIFECYCLE` · `ORC-NEGATIVE-CLOSURE`, `ORC-RECOVERY-ATOMICITY` · `CFP-FENCE-AFTER-TAKEOVER` | `B-ABSENT` + `B-RECOVERY` + `B-HANDOFF`; `PASS-REFUSAL` for fake path and `PASS-STAGE` for S2 continuation. | `BLOCK-ALL`; disabled fake path that breaks S2, adds hidden route, or loses author data blocks. |
| `S3-JRN-001` | Six-step Stage 3 journey—current passage/request, fake full path, edit/accept/reject, refusal/conflict/interruption, fake-off S2 repeat—is executable; `OWN-REL` with `OWN-PLAIN`, `OWN-CTX`, `OWN-AGENT`, `OWN-CORE`, `OWN-WEB`. | `DVG-02`, `DVG-03`, `DVG-04`, `DVG-05`, `DVG-06`, `DVG-07`, `DVG-08`, `DVG-11`, `DVG-13` | `EC-01/02/03/04/05/07/08`; `EV-CP/INT/SE/SR` | `FX-FAKE-MODEL`, `FX-CORE-PROPOSAL`, `FX-ABSENT-EXECUTION`, `FX-RECOVERY-EDITOR` · all fake/proposal/unknown/fence/absence points · `SCH-NORMAL/UNKNOWN/FENCE/CRASH/ABSENT` · `ORC-CROSSWALK-COMPLETENESS`, `ORC-ATOMIC-AUTHORITY`, `ORC-NEGATIVE-CLOSURE` · `CFP-DISPATCH-AFTER-IO-BEFORE-CONFIRMATION` | `B-FAKE` + `B-CORE` + `B-ABSENT` + `B-HANDOFF`; `PASS-STAGE`. | `BLOCK-ALL`; any journey step, S2 repeat, proposal decision, or unknown recovery gap blocks Stage 3. |
...
| `S3-EVD-006` | Fake limitation evidence states exactly what is and is not claimed; `OWN-MODEL`, `OWN-REL`, `OWN-DVG`. | `DVG-04`, `DVG-05`, `DVG-08`, `DVG-13` | `EC-01/08`; `EV-CP/RDA/SE` | `FX-FAKE-MODEL`, `FX-REAL-MODEL-ADVISORY`, `FX-HANDOFF` · dispatch/unknown/crosswalk points · `SCH-UNKNOWN/DRIFT` · `ORC-DISPATCH-DISCLOSURE`, `ORC-CROSSWALK-COMPLETENESS` · `CFP-CONTRACT-DRIFT` | `B-FAKE` + `B-REAL-ADVISORY` + `B-HANDOFF`; `PASS-POS` for limitation record; real observation remains `advisory`. | `BLOCK-ALL` if deterministic fake proof or limitation disclosure is absent; advisory cannot upgrade. |
| `S3-EVD-007` | S2 regression evidence proves fake unavailable does not impair dependable editor; `OWN-REL`, `OWN-WEB`, `OWN-CORE`, `OWN-PG`. | `DVG-01`, `DVG-03`, `DVG-07`, `DVG-09`, `DVG-11`, `DVG-13` | `EC-01/02/03/04/05`; `EV-CP/IT/INT/SE` | `FX-ABSENT-EXECUTION`, `FX-RECOVERY-EDITOR`, `FX-HANDOFF` · absence/crash/replay/core points · `SCH-ABSENT/CRASH/REPLAY` · `ORC-NEGATIVE-CLOSURE`, `ORC-RECOVERY-ATOMICITY` · `CFP-FENCE-AFTER-TAKEOVER` | `B-ABSENT` + `B-RECOVERY` + `B-HANDOFF`; `PASS-STAGE`. | `BLOCK-ALL`. |
...
| `S4-REQ-006` | Real author can write/request/inspect/edit/Accept/Reject/recover all StoryOS-owned facts; `OWN-REL`, `OWN-WEB`, `OWN-CORE`, `OWN-ADM`, `OWN-CTX`, `OWN-RET`. | `DVG-02`, `DVG-03`, `DVG-04`, `DVG-05`, `DVG-07`, `DVG-08`, `DVG-10`, `DVG-11`, `DVG-13` | `EC-02/03/04/05/07/08`; `EV-CP/INT/PRD/RDA/SE/SR` | `FX-REAL-MODEL-ADVISORY`, `FX-CORE-PROPOSAL`, `FX-RECOVERY-EDITOR`, `FX-RESTORE-LIFECYCLE` · all relevant author/manifest/unknown/recovery/lifecycle points · `SCH-NORMAL/UNKNOWN/CRASH/FENCE/RESTORE/SCOPE` · `ORC-ATOMIC-AUTHORITY`, `ORC-RECOVERY-ATOMICITY`, `ORC-RESTORE-LIFECYCLE` · `CFP-EDITOR-AFTER-SETTLEMENT-BEFORE-ACK` | `B-REAL-ADVISORY` + All applicable bundles; `PASS-STAGE` after S4 journey and mandatory evidence. | `BLOCK-ALL`; any uninspectable fact, missing decision, recovery gap, or Provider claim substituted for StoryOS evidence blocks. |
...
| `S4-JRN-001` | Seven-step Stage 4 journey—standalone editor, bounded request, pre-egress manifest, edit/accept/reject, post-dispatch unknown, reconciliation/recovery, model-disabled S2 repeat—is executable; `OWN-REL` and all Stage 4 owners. | `DVG-01`, `DVG-02`, `DVG-03`, `DVG-04`, `DVG-05`, `DVG-07`, `DVG-08`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-13` | `EC-01/02/03/04/05/07/08`; `EV-CP/INT/PRD/RDA/SE/SR` | `FX-REAL-MODEL-ADVISORY`, `FX-CONTEXT-DISCLOSURE`, `FX-CORE-PROPOSAL`, `FX-RECOVERY-EDITOR`, `FX-ABSENT-EXECUTION` · all route/manifest/unknown/proposal/recovery/absence points · `SCH-NORMAL/UNKNOWN/CRASH/FENCE/RESTORE/SCOPE/ABSENT` · `ORC-CROSSWALK-COMPLETENESS`, `ORC-OUTCOME-UNKNOWN`, `ORC-NEGATIVE-CLOSURE` · `CFP-DISPATCH-AFTER-IO-BEFORE-CONFIRMATION` | `B-HANDOFF` + All applicable bundles; `PASS-STAGE`. | `BLOCK-ALL`; any missing manifest, unknown recovery, author decision, S2 repeat, or absent-capability refusal blocks Stage 4. |
...
| `S4-EVD-007` | S2 regression and unavailable-model evidence prove manual editor continuity and no hidden fallback; `OWN-REL`, `OWN-WEB`, `OWN-CORE`, `OWN-PG`, `OWN-AGENT`. | `DVG-01`, `DVG-03`, `DVG-07`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-13` | `EC-01/02/03/04/05`; `EV-CP/IT/INT/PRD/SE` | `FX-ABSENT-EXECUTION`, `FX-RECOVERY-EDITOR`, `FX-RESTORE-LIFECYCLE` · absence/crash/replay/restore/lifecycle points · `SCH-ABSENT/CRASH/REPLAY/RESTORE` · `ORC-NEGATIVE-CLOSURE`, `ORC-RECOVERY-ATOMICITY`, `ORC-RESTORE-LIFECYCLE` · `CFP-FENCE-AFTER-TAKEOVER` | `B-ABSENT` + `B-RECOVERY` + `B-RESTORE` + `B-HANDOFF`; `PASS-STAGE`. | `BLOCK-ALL`. |
...
| `HND-002` | Implementation handoff is exactly one bounded Stage 1 issue after #60; no implementation is hidden in this contract; `OWN-REL`, `OWN-GOV`, `OWN-STAGE`. | `DVG-01`, `DVG-02`, `DVG-03`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-13` | `EC-01/02/03/04/05`; `EV-CP/SE` | `FX-HANDOFF`, `FX-CONTRACT-R1` · source/drift/scope/core/recovery/lifecycle points · `SCH-DRIFT/NORMAL/ABSENT` · `ORC-CROSSWALK-COMPLETENESS` · `CFP-CONTRACT-DRIFT` | `B-HANDOFF`; `PASS-STAGE` for a bounded handoff record only. | `BLOCK-ALL`; an unbounded implementation scope, extra issue, or product change blocks. |
| `HND-003` | Stage implementation evidence is complete, attributable to the exact baseline, and covers all applicable gates; `OWN-REL` with each stage owner. | `DVG-01`, `DVG-02`, `DVG-03`, `DVG-04`, `DVG-05`, `DVG-06`, `DVG-07`, `DVG-08`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-12`, `DVG-13` | `EC-01/02/03/04/05/06/07/08`; `EV-CP/IT/INT/PRD/RDA/SE` | `FX-HANDOFF` · `CFP-CONTRACT-DRIFT` · `SCH-DRIFT`, `SCH-NORMAL`, and the applicable stage schedules · `ORC-CROSSWALK-COMPLETENESS` plus row oracle | All row bundles + `B-HANDOFF`; `PASS-STAGE`. | `BLOCK-ALL`; one missing/stale/unreplayable mandatory bundle blocks. |
| `HND-004` | Stage release requires author journey plus mandatory pass and records the resulting main/next input; `OWN-REL`/`OWN-STAGE`. | `DVG-01`, `DVG-02`, `DVG-03`, `DVG-04`, `DVG-05`, `DVG-06`, `DVG-07`, `DVG-08`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-12`, `DVG-13` | `EC-01/02/03/04/05/06/07/08`; `EV-INT/PRD/SE/SR` | `FX-HANDOFF` plus stage journey fixture · `CFP-CONTRACT-DRIFT` · `SCH-NORMAL` plus the applicable stage schedules · `ORC-CROSSWALK-COMPLETENESS` | `B-HANDOFF` + stage bundles; `PASS-STAGE`. | `BLOCK-ALL`; passing gates without journey, or journey without mandatory pass, is not release. |
...
| `HND-006` | Serial direction remains #62 → #60 → #77; no parallel frontier or successor execution is implied; `OWN-REL`/`OWN-GOV`. | `DVG-13` | `EC-01`; `EV-CP/SE` | `FX-HANDOFF` · contract-drift/crosswalk points · `SCH-DRIFT/NORMAL` · `ORC-CROSSWALK-COMPLETENESS` · `CFP-CONTRACT-DRIFT` | `B-HANDOFF`; `PASS-POS`. | `BLOCK-ALL`; any changed frontier, unclaimed successor, or missing handoff record blocks. |
...
| After Core/Admission settlement but before ack/Event | Durable settled fact; browser/server recovery reconciles from it. | Exact idempotent replay/convergence. |
...
- [CONTEXT.md](../../CONTEXT.md), the live Wayfinder #1 map, #60, #62, #77,
  and [issue-tracker instructions](../agents/issue-tracker.md);
...
claim, edit, comment on, close, or execute #77, merge a PR, update the
