...
- Exact release-source snapshot consumed: `cca8e646f137bccc27414014470211451a1ac474`
- Exact source tree consumed: `04b33642b800cd60345bf75bf366c1c08c59a1c3`
...
blocks a stage or release. Section 12 and the [product delivery proof
selection](product-delivery-proof-selection.md) form one normative crosswalk.
The companion binds the accepted sources, completes coverage, and defines the
positive continuation profiles. It adds no verification runtime.

[Eval](eval-evidence-foundation.md) is outside MVP. No current stage selects
an Eval page, API, assessment, fixture, oracle, or evidence bundle. Ordinary
Agent input/output, Trace, usage, timing, and recovery proof remain required
by their existing owners. Historical Stage 1 exclusions remain unchanged.

## 1. Authority, claim ceiling, and non-scope

### 1.1 Authority order

The accepted release baseline and its continuation are the source of `REL-*`,
`HND-*`, and the Stage 1 through Stage 9 meanings. The owner contracts listed in section 4
remain the source of domain, authority, protocol, storage, retention,
disclosure, Agent, model, Tool, MCP, editor, and trust semantics. This
...
the StoryOS-owned boundary. Real-destination observations do not become deterministic oracles. The
accepted Stage 4 and Stage 5 journeys nevertheless require actual qualified
integration evidence: missing required observations block those stages.
Host facts use `EV-INT`; attributable Provider observations use `EV-RDA`. A
...
- change the accepted stages, author journeys, product priority, required or absent
...
- implement Rust, TypeScript, browser, server, worker, database, migration,
  ...
  Memory, Skill, or Subrun execution;
- turn a Proposal, transcript, browser projection, local journal, model output,
  Tool/MCP result, or advisory projection into authoritative state;
- make a real Provider's behavior a deterministic oracle; or
- authorize a product Claim, a new ticket breakdown, a stage release, or a
  change to `.reference/**`.
...
boundaries or stand in for Provider internals. A required real integration also supplies `EV-INT` for StoryOS-owned
integration facts. Provider-internal observations remain advisory and cannot
replace the fake/oracle proof.
...
used to make the complete crosswalk auditable without weakening the result
semantics.

### 3.3 No circular evidence

An implementation result, UI state, generated report, or other projection may
...
| --- | --- |
| `OWN-REL` | [AI-Independent Editor-First Release Baseline and Handoff Criteria](ai-independent-editor-first-release-baseline-and-handoff-criteria.md) and [Product Delivery Continuation](storyos-product-delivery-continuation.md), including `REL-*`, stage, evidence, and handoff meanings. |
...
| `OWN-MEM` | [Fiction Memory and Research Provenance Semantics](fiction-memory-and-research-provenance-semantics.md), including generated Memory Publications, Notes, independent conversation settings, Research evidence, real source/copy restrictions, and index rebuild. |
...
| `OWN-MODEL` | [Specify ModelGateway and Model-Routing Semantics](https://github.com/FrankQDWang/StoryOS/issues/50), the Model Gateway/Provider owner, including [ADR 0033](../adr/0033-use-volcengine-responses-for-the-first-real-model-path.md) for typed output, continuation, active compaction, and bounded recovery. Provider internals and model-quality claims remain outside deterministic proof. |
...
| `OWN-DVG` | This document, [Product Delivery Proof Selection](product-delivery-proof-selection.md), and [ADR 0012](../adr/0012-adopt-deterministic-contract-verification.md), limited to proof selection, oracle, fault-point, schedule, bundle, disposition, and handoff semantics. |
...
| `FX-FAKE-MODEL` | Contract-faithful fake model through the normal Host/assembly/manifest/Attempt/fence/Proposal/Receipt path, including typed outputs, conversation/settings admission, continuation, compaction, refusal, and bounded unknown recovery. Companion sections 2.1 and 2.2 define the exact cases. |
| `FX-REAL-MODEL-ADVISORY` | One qualified Volcengine Agent Plan Responses endpoint/model/account route through Provider-neutral contracts. Use permitted synthetic content, separately qualified capabilities and combinations, and truthful reports; no claim about Provider internals or quality. |
...
| `CFP-LIFECYCLE-BEFORE-INVALIDATION` | An applicable source/copy restriction or redaction/Tombstone/Archive/Deletion has not committed; current eligibility remains owner-defined. |
...
| `CFP-CONTEXT-BEFORE-INSTALL` | A compaction result is staged for a later request. Validate its exact inputs and current admission before installation; preserve prior Messages, Snapshots, manifests, Attempts, and required recovery bytes. |
| `CFP-MODEL-AFTER-SUCCESSOR-FENCE` | The predecessor fence and spent one-successor allowance are durable before the additional Attempt. Crash/repeated recovery cannot reset the allowance; late predecessor output is evidence only. |
| `CFP-HOSTED-BEFORE-ADMISSION` | Before final dispatch, validate the entire enabled hosted set, explicit intake, destinations, effect ceilings, exact Approval, enforced bounds, and worst-case reservation. A missing condition prevents submission. |
| `CFP-MEMORY-BEFORE-PUBLICATION` | Staged generated output precedes settlement under the Project publication guard. Recheck lease, expected generation, exact inputs, complete set, and current source/copy restrictions; failure exposes no partial set. |
| `CFP-MEMORY-AFTER-PUBLICATION` | Publication/members/Heads/pointer, processed inputs/Notes, outcome, Activity, and invalidation have committed before acknowledgement. Replay returns that settlement without a second publication or lost pending Note. |
| `CFP-MEMORY-BEFORE-BYTE-ACQUISITION` | Publication, byte-dependent work acquisition, and cleanup eligibility meet under the same guard. Either bytes remain protected for admitted work or the durable availability fence prevents new acquisition before physical cleanup. |
...
| `B-S3-MANDATORY-SET` | Exact Stage 3 mandatory bundle set, formed from the complete Stage 2 set plus the fake-model and absence boundaries; members are `B-CONTRACT`, `B-SCOPE`, `B-EDITOR`, `B-CORE`, `B-RECOVERY`, `B-REPLAY`, `B-RESTORE`, `B-CONTEXT`, `B-FAKE`, `B-ABSENT`, `B-MEASURE`, and `B-HANDOFF`. It contains no real-destination advisory bundle. |
| `B-S4-MANDATORY-SET` | Exact Stage 4 mandatory bundle set, formed from the complete Stage 3 deterministic set plus one real-destination advisory boundary; members are `B-CONTRACT`, `B-SCOPE`, `B-EDITOR`, `B-CORE`, `B-RECOVERY`, `B-REPLAY`, `B-RESTORE`, `B-CONTEXT`, `B-FAKE`, `B-ABSENT`, `B-MEASURE`, `B-HANDOFF`, and `B-REAL-ADVISORY`. `B-FAKE` is a contract-faithful deterministic proof fixture, not a second Provider or user-facing route; real-destination evidence cannot replace it. |
...
| `SCH-LIFECYCLE` | Start eligible record → commit an applicable source/copy restriction, redaction, archive, or deletion at the selected cut → attempt current read/retrieval/cache/export/egress → inspect historical truth, gap, invalidation, and non-revival. |
...
| --- | --- | --- | --- |
| `DVG-01` Contract source, generated schema/catalog, golden wire, same-release identity, bounds, and drift | `OWN-PROTO` + `OWN-PG` + `OWN-GOV`; proves source-to-generated-to-wire/catalog parity, exact release identity, accepted limits, and fail-closed drift. For `storyos.author-edit-batch.release-1.preview.v1`, it checks the #70-owned structured source, current `storyos.editor-contract.release-1.v3` legacy batch mapping, captured synthetic evidence binding for legacy primitives only, and document projections; it does not treat prose presence as semantic proof. | `FX-CONTRACT-R1` · `CFP-CONTRACT-BEFORE-GENERATION`, `CFP-CONTRACT-AFTER-GENERATION-BEFORE-WIRE`, `CFP-CONTRACT-DRIFT` · `SCH-DRIFT` · `ORC-CONTRACT`. | `B-CONTRACT`; `PASS-POS` for parity or `PASS-REFUSAL`/`PASS-HOLD` for drift. `BLOCK-ALL`. |
...
| `DVG-05` Model Gateway, Provider, Tool, MCP, and MCP App mediation | `OWN-CTX` + `OWN-MODEL` + `OWN-TOOL` + `OWN-MCP` + `OWN-APP` + `OWN-TRUST`; proves destination identity/grant/capability/credential/policy, manifest, Attempt, disclosure, and no direct authority. Positive Tool and App cases use the companion profiles. For Stage 3/4 absent execution it proves refusal and zero fallback, not execution. | `FX-FAKE-MODEL`, `FX-REAL-MODEL-ADVISORY`, `FX-ABSENT-EXECUTION` · `CFP-MANIFEST-AFTER-COMMIT-BEFORE-EGRESS`, `CFP-DISPATCH-AFTER-CLAIM-BEFORE-IO`, `CFP-DISPATCH-AFTER-IO-BEFORE-CONFIRMATION`, `CFP-LATE-RESULT` · `SCH-UNKNOWN`, `SCH-ABSENT`, `SCH-SCOPE` · `ORC-DISPATCH-DISCLOSURE`. | `B-FAKE` for deterministic proof, `B-REAL-ADVISORY` for advisory observation, `B-ABSENT` for excluded capabilities; `PASS-POS`, `PASS-UNKNOWN`, or `PASS-REFUSAL`. `BLOCK-ALL`; advisory never upgrades. |
| `DVG-06` AgentRun, Subrun, Mailbox, and finalization boundary | `OWN-AGENT` + `OWN-SUBRUN` + `OWN-RET` + `OWN-CORE`; proves one general Project Agent Loop, bounded run/step/mailbox settlement, finalization, fence, and no task-specific authority path. The companion selects positive Subrun proof when that capability is delivered. | `FX-FAKE-MODEL`, `FX-ABSENT-EXECUTION` · `CFP-MAILBOX-BEFORE-SEAL`, `CFP-MAILBOX-AFTER-SEAL`, `CFP-MAILBOX-LATE-DUPLICATE`, `CFP-FENCE-AFTER-TAKEOVER` · `SCH-FENCE`, `SCH-REPLAY`, `SCH-ABSENT` · `ORC-RUN-FINALIZATION`. | `B-FAKE` or `B-ABSENT`; `PASS-POS`, `PASS-REFUSAL`, or `PASS-HOLD`. `BLOCK-ALL`. |
...
This historical profile retains its exact Stage 1 selection and claim ceiling.
Its completed binding and implementation evidence stay with the original
Stage 1 owners. This revision does not reopen that handoff.
...
- **`DVG-04` context corpus.** For every source role, exercise all seven
  ...
  actual source/copy restrictions, redaction, bounded excerpt/summary, dynamic retrieval,
  ...
  no prohibited new source read or StoryOS-controlled submission. Recorded
  history is not a fresh read of every source it mentions. Ordinary corrections
  remain ordered Messages; no semantic exclusion or precise-forgetting oracle
  is permitted. Hosted internal consumption is outside the Host observation boundary.
...
- **`DVG-08` unknown/reconciliation corpus.** For model, Tool/MCP, research,
  ...
  `Committed | Rejected | RequiresReconfirmation | StillUnknown` result. Compare
  the complete Receipt, Activity, Journal, queue, capsule, and authority
  snapshot. Only the safe GET may repeat while the Journal is unresolved. The
  Query may complete the already-admitted command; it still creates no new
  reconciliation command or POST Attempt.
- **`DVG-09` replay/retention corpus.** Exercise cursor resume, retained
  ...
  actual retained-copy restriction, redaction/Tombstone logical effect, and
  applicable cache/projection/Provider-continuity invalidation. Ordinary source
  edits and Memory settings do not erase recorded history or reset continuation. The oracle keeps
...
6. **Manifest-before-egress:** no external bytes leave before current
   ...
   returned external values re-enter Context Assembly before a later
   StoryOS-controlled submission. Whole hosted-operation admission precedes
   submission; invisible internal steps do not create invented Host gates.
...
9. **Non-revival:** owner-defined restrictions on exact retained payloads and
   covered copies survive cache, export, and recovery. Reconstruction can
   publish new generated revisions from allowed inputs but cannot lift old
   payload fences. An ordinary source edit, correction, or Memory Note is not
   a covered-copy restriction or proof of erased semantic influence.
10. **No hidden authority:** Tool, MCP, App, Provider, cache, model
...
| `ORC-ATOMIC-AUTHORITY` | A Core write set is all-or-nothing; typed Receipt/Author Action/Proposal result matches durable state and exact idempotency. An `ApplyAuthorEdit.AuthoritativeApplied` result has one exact Project Activity relation; each zero-authority Author Edit result has one typed Receipt and zero Project Activity. `ApplyAuthorEdit.ProposalRevised` has one Forward Author Action and a Proposal Revision with no Authoritative Commit. |
| `ORC-CONTEXT-DISCLOSURE` | Only eligible bounded input reaches a committed manifest and matching StoryOS-controlled submission; no manifest means zero IO. Current reads, recorded history, active compaction, and real copy restrictions follow companion section 2.2; there is no model-influence oracle. |
...
### 11.1 First four stages and the accepted continuation

The first four stages retain the identities accepted by [Define the AI-Independent Editor-First Release Baseline and Handoff Criteria](https://github.com/FrankQDWang/StoryOS/issues/62). The proof order is:
...
4. **Stage 4 — one real external-model route:** one qualified Volcengine Agent
   Plan Responses destination under the same identity, disclosure, Attempt/unknown/recovery,
   Proposal, and S2 fallback boundaries; no Provider-internal or literary claim.

Stages 5 through 9 deliver governed Tools/research, Skills, Memory, MCP Apps,
and complete orchestration. Their positive proof selections and
release branches are defined in the companion. Stage 4 is an intermediate gate.
...
variants: Tool, MCP, research, embedding, Memory, Skill, and Subrun
execution. The corresponding proof is a negative/refusal proof through
`DVG-04`, `DVG-05`, `DVG-06`, and `DVG-11`; a fake bounded variant
...
retention, training, or literary quality. `EV-INT` proves actual StoryOS
integration; `EV-RDA` records attributable Provider observations. Required
Agent Plan capability or combination gaps block Stage 4. Native compaction
and an explicit-cache hit are not universal gates: the accepted validated
Host-managed summary and optional-cache behavior remain valid. Conversation
settings are recorded from Stage 3, but Memory execution starts only in Stage 7.
...
| `WALK-06` Fake-model Stage 3 | Bounded current request → real Host/Scope/Context Assembly → atomic conversation/settings → typed Decision/continuation/compaction → fake Attempt/fence/expiry/lookup/bounded unknown recovery → Proposal/author decision → disable fake and repeat S2. | `FX-FAKE-MODEL`, `FX-RECOVERY-EDITOR` · `SCH-NORMAL`, `SCH-UNKNOWN`, `SCH-FENCE` | `DVG-02`, `DVG-03`, `DVG-04`, `DVG-05`, `DVG-06`, `DVG-07`, `DVG-08`, `DVG-11`, `DVG-13` |
| `WALK-07` One-real-model Stage 4 | Registration/binding/capability/credential/policy → same assembly/manifest → qualified Agent Plan typed continuation and capability combinations → real Attempt → expiry/lookup/bounded unknown recovery → Proposal/author decision → disable and repeat S2. | `FX-REAL-MODEL-ADVISORY`, `FX-FAKE-MODEL` · `SCH-NORMAL`, `SCH-UNKNOWN`, `SCH-SCOPE`, `SCH-FENCE` | `DVG-02`, `DVG-03`, `DVG-04`, `DVG-05`, `DVG-07`, `DVG-08`, `DVG-11`, `DVG-13` |
| `WALK-08` Absent execution | Request each excluded capability → refusal boundary → no hidden route, egress, authority, or model run. | `FX-ABSENT-EXECUTION` · `SCH-ABSENT`, `SCH-SCOPE`, `SCH-LIFECYCLE` | `DVG-04`, `DVG-05`, `DVG-06`, `DVG-11` |
...
bundle, disposition, and block token against this document and its companion.

The finite stage-map selectors below are also registered crosswalk tokens.
`SMAP-STAGE-1` through `SMAP-STAGE-9` are mandatory implementation-evidence
maps: each selects its registered row here or in the companion and never contains `EV-SR`.
`SMAP-EVALUATED-MANDATORY-STAGE` selects exactly one of those nine maps from
...
Contract Fault Points, schedules, oracles, and mandatory bundle set. The companion section 2.1 adds the exact case-group selections to each
listed release row; its current cases are mandatory, including in inherited
stage maps. In the
...
adds to its matching subfield. It never expands to the union of all stages or to a bundle wildcard.

The separate `SMAP-RELEASE-STAGE-1` through `SMAP-RELEASE-STAGE-9` selectors
...
| `SMAP-STAGE-2` | Stage 2 (mandatory implementation evidence) | `DVG-01`, `DVG-02`, `DVG-03`, `DVG-04`, `DVG-07`, `DVG-08`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-13` | `EC-01/02/03/04/05/06/07`; `EV-CP/IT/INT/PRD/SE` | `FX-ABSENT-EXECUTION`, `FX-CONTEXT-DISCLOSURE`, `FX-CONTRACT-R1`, `FX-CORE-PROPOSAL`, `FX-EDITOR-IME`, `FX-HANDOFF`, `FX-JOURNAL-GROUP`, `FX-LONG-SESSION`, `FX-RECOVERY-EDITOR`, `FX-REPLAY-RETENTION`, `FX-RESTORE-LIFECYCLE`, `FX-SCOPE-2U2P`, `FX-PRODUCTION-EDITOR` | `CFP-ADMISSION-BEFORE-CORE`, `CFP-ADMISSION-EXPIRY`, `CFP-CONTRACT-DRIFT`, `CFP-CORE-AFTER-COMMIT-BEFORE-ACK`, `CFP-CORE-BEFORE-COMMIT`, `CFP-DELETE-AFTER-SETTLEMENT`, `CFP-DELETE-BEFORE-SETTLEMENT`, `CFP-EDITOR-AFTER-JOURNAL-BEFORE-GROUP`, `CFP-EDITOR-AFTER-SETTLEMENT-BEFORE-ACK`, `CFP-EDITOR-BEFORE-GROUP-ADMISSION`, `CFP-EDITOR-BEFORE-JOURNAL-DURABILITY`, `CFP-FENCE-AFTER-TAKEOVER`, `CFP-LIFECYCLE-AFTER-INVALIDATION-BEFORE-CLEANUP`, `CFP-MANIFEST-BEFORE-COMMIT`, `CFP-REPLAY-AFTER-GENERATION-SNAPSHOT`, `CFP-REPLAY-BEFORE-COMPACTION`, `CFP-REPLAY-BELOW-FLOOR`, `CFP-RESTORE-AFTER-VISIBILITY`, `CFP-RESTORE-BEFORE-VISIBILITY`, `CFP-RESTORE-STAGING`, `CFP-SCOPE-BEFORE-QUERY`, `CFP-UNDO-BEFORE-SETTLEMENT`, `CFP-EDITOR-AFTER-ADMISSION-BEFORE-CORE`, `CFP-EDITOR-AFTER-OUTCOME-RESPONSE-BEFORE-JOURNAL` | `SCH-ABSENT/CRASH/DRIFT/FENCE/LIFECYCLE/LONG/NORMAL/REORDER/REPLAY/RESTORE/SCOPE/UNKNOWN` | `ORC-ATOMIC-AUTHORITY`, `ORC-CONTEXT-DISCLOSURE`, `ORC-CROSSWALK-COMPLETENESS`, `ORC-EDITOR-JOURNAL`, `ORC-NEGATIVE-CLOSURE`, `ORC-OUTCOME-UNKNOWN`, `ORC-RECOVERY-ATOMICITY`, `ORC-REPLAY-TRUTH`, `ORC-RESTORE-LIFECYCLE`, `ORC-SCOPE`, `ORC-PRODUCTION-EDITOR`, `ORC-CONTRACT` | `B-S2-MANDATORY-SET` |
| `SMAP-STAGE-3` | Stage 3 (mandatory implementation evidence) | `DVG-01`, `DVG-02`, `DVG-03`, `DVG-04`, `DVG-05`, `DVG-06`, `DVG-07`, `DVG-08`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-13` | `EC-01/02/03/04/05/06/07/08`; `EV-CP/IT/INT/PRD/SE` (no real-destination advisory or stage-release output) | `FX-ABSENT-EXECUTION`, `FX-CONTEXT-DISCLOSURE`, `FX-CONTRACT-R1`, `FX-CORE-PROPOSAL`, `FX-EDITOR-IME`, `FX-FAKE-MODEL`, `FX-HANDOFF`, `FX-JOURNAL-GROUP`, `FX-LONG-SESSION`, `FX-RECOVERY-EDITOR`, `FX-REPLAY-RETENTION`, `FX-RESTORE-LIFECYCLE`, `FX-SCOPE-2U2P`, `FX-PRODUCTION-EDITOR`, `FX-PROPOSAL-INTERACTION` | `CFP-ADMISSION-BEFORE-CORE`, `CFP-ADMISSION-EXPIRY`, `CFP-CONTRACT-DRIFT`, `CFP-CORE-AFTER-COMMIT-BEFORE-ACK`, `CFP-CORE-BEFORE-COMMIT`, `CFP-DELETE-AFTER-SETTLEMENT`, `CFP-DELETE-BEFORE-SETTLEMENT`, `CFP-DISPATCH-AFTER-IO-BEFORE-CONFIRMATION`, `CFP-EDITOR-AFTER-JOURNAL-BEFORE-GROUP`, `CFP-EDITOR-AFTER-SETTLEMENT-BEFORE-ACK`, `CFP-EDITOR-BEFORE-GROUP-ADMISSION`, `CFP-EDITOR-BEFORE-JOURNAL-DURABILITY`, `CFP-FENCE-AFTER-TAKEOVER`, `CFP-LIFECYCLE-AFTER-INVALIDATION-BEFORE-CLEANUP`, `CFP-MANIFEST-AFTER-COMMIT-BEFORE-EGRESS`, `CFP-MANIFEST-BEFORE-COMMIT`, `CFP-PROPOSAL-AFTER-ACCEPTANCE-BEFORE-RECEIPT`, `CFP-PROPOSAL-BEFORE-DECISION`, `CFP-REPLAY-AFTER-GENERATION-SNAPSHOT`, `CFP-REPLAY-BEFORE-COMPACTION`, `CFP-REPLAY-BELOW-FLOOR`, `CFP-RESTORE-AFTER-VISIBILITY`, `CFP-RESTORE-BEFORE-VISIBILITY`, `CFP-RESTORE-STAGING`, `CFP-SCOPE-BEFORE-QUERY`, `CFP-UNDO-BEFORE-SETTLEMENT`, `CFP-MAILBOX-BEFORE-SEAL`, `CFP-MAILBOX-AFTER-SEAL`, `CFP-MAILBOX-LATE-DUPLICATE`, `CFP-EDITOR-AFTER-ADMISSION-BEFORE-CORE`, `CFP-EDITOR-AFTER-OUTCOME-RESPONSE-BEFORE-JOURNAL`, `CFP-CONVERSATION-BEFORE-ADMISSION`, `CFP-MODEL-BEFORE-DECISION`, `CFP-MODEL-AFTER-DECISION-BEFORE-CONTINUATION`, `CFP-CONTEXT-BEFORE-INSTALL`, `CFP-MODEL-AFTER-SUCCESSOR-FENCE`, `CFP-DISPATCH-AFTER-CLAIM-BEFORE-IO`, `CFP-RECONCILIATION-BEFORE-SETTLEMENT`, `CFP-LATE-RESULT` | `SCH-ABSENT/CRASH/DRIFT/FENCE/LIFECYCLE/LONG/NORMAL/REORDER/REPLAY/RESTORE/SCOPE/UNKNOWN` | `ORC-ATOMIC-AUTHORITY`, `ORC-CONTEXT-DISCLOSURE`, `ORC-CROSSWALK-COMPLETENESS`, `ORC-DISPATCH-DISCLOSURE`, `ORC-EDITOR-JOURNAL`, `ORC-NEGATIVE-CLOSURE`, `ORC-OUTCOME-UNKNOWN`, `ORC-RECOVERY-ATOMICITY`, `ORC-REPLAY-TRUTH`, `ORC-RESTORE-LIFECYCLE`, `ORC-RUN-FINALIZATION`, `ORC-SCOPE`, `ORC-PRODUCTION-EDITOR`, `ORC-PROPOSAL-INTERACTION`, `ORC-CONTRACT` | `B-S3-MANDATORY-SET` |
| `SMAP-STAGE-4` | Stage 4 (mandatory implementation evidence) | `DVG-01`, `DVG-02`, `DVG-03`, `DVG-04`, `DVG-05`, `DVG-06`, `DVG-07`, `DVG-08`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-13` | `EC-01/02/03/04/05/06/07/08`; `EV-CP/IT/INT/PRD/RDA/SE` | `FX-ABSENT-EXECUTION`, `FX-CONTEXT-DISCLOSURE`, `FX-CONTRACT-R1`, `FX-CORE-PROPOSAL`, `FX-EDITOR-IME`, `FX-FAKE-MODEL`, `FX-HANDOFF`, `FX-JOURNAL-GROUP`, `FX-LONG-SESSION`, `FX-REAL-MODEL-ADVISORY`, `FX-RECOVERY-EDITOR`, `FX-REPLAY-RETENTION`, `FX-RESTORE-LIFECYCLE`, `FX-SCOPE-2U2P`, `FX-PRODUCTION-EDITOR`, `FX-PROPOSAL-INTERACTION` | `CFP-ADMISSION-BEFORE-CORE`, `CFP-ADMISSION-EXPIRY`, `CFP-CONTRACT-DRIFT`, `CFP-CORE-AFTER-COMMIT-BEFORE-ACK`, `CFP-CORE-BEFORE-COMMIT`, `CFP-DELETE-AFTER-SETTLEMENT`, `CFP-DELETE-BEFORE-SETTLEMENT`, `CFP-DISPATCH-AFTER-CLAIM-BEFORE-IO`, `CFP-DISPATCH-AFTER-IO-BEFORE-CONFIRMATION`, `CFP-DISPATCH-BEFORE-CLAIM`, `CFP-EDITOR-AFTER-JOURNAL-BEFORE-GROUP`, `CFP-EDITOR-AFTER-SETTLEMENT-BEFORE-ACK`, `CFP-EDITOR-BEFORE-GROUP-ADMISSION`, `CFP-EDITOR-BEFORE-JOURNAL-DURABILITY`, `CFP-FENCE-AFTER-TAKEOVER`, `CFP-LATE-RESULT`, `CFP-LIFECYCLE-AFTER-INVALIDATION-BEFORE-CLEANUP`, `CFP-MANIFEST-AFTER-COMMIT-BEFORE-EGRESS`, `CFP-MANIFEST-BEFORE-COMMIT`, `CFP-PROPOSAL-AFTER-ACCEPTANCE-BEFORE-RECEIPT`, `CFP-PROPOSAL-BEFORE-DECISION`, `CFP-RECONCILIATION-BEFORE-SETTLEMENT`, `CFP-REPLAY-AFTER-GENERATION-SNAPSHOT`, `CFP-REPLAY-BEFORE-COMPACTION`, `CFP-REPLAY-BELOW-FLOOR`, `CFP-RESTORE-AFTER-VISIBILITY`, `CFP-RESTORE-BEFORE-VISIBILITY`, `CFP-RESTORE-STAGING`, `CFP-SCOPE-BEFORE-QUERY`, `CFP-UNDO-BEFORE-SETTLEMENT`, `CFP-MAILBOX-BEFORE-SEAL`, `CFP-MAILBOX-AFTER-SEAL`, `CFP-MAILBOX-LATE-DUPLICATE`, `CFP-EDITOR-AFTER-ADMISSION-BEFORE-CORE`, `CFP-EDITOR-AFTER-OUTCOME-RESPONSE-BEFORE-JOURNAL`, `CFP-CONVERSATION-BEFORE-ADMISSION`, `CFP-MODEL-BEFORE-DECISION`, `CFP-MODEL-AFTER-DECISION-BEFORE-CONTINUATION`, `CFP-CONTEXT-BEFORE-INSTALL`, `CFP-MODEL-AFTER-SUCCESSOR-FENCE` | `SCH-ABSENT/CRASH/DRIFT/FENCE/LIFECYCLE/LONG/NORMAL/REORDER/REPLAY/RESTORE/SCOPE/UNKNOWN` | `ORC-ATOMIC-AUTHORITY`, `ORC-CONTEXT-DISCLOSURE`, `ORC-CONTRACT`, `ORC-CROSSWALK-COMPLETENESS`, `ORC-DISPATCH-DISCLOSURE`, `ORC-EDITOR-JOURNAL`, `ORC-NEGATIVE-CLOSURE`, `ORC-OUTCOME-UNKNOWN`, `ORC-RECOVERY-ATOMICITY`, `ORC-REPLAY-TRUTH`, `ORC-RESTORE-LIFECYCLE`, `ORC-RUN-FINALIZATION`, `ORC-SCOPE`, `ORC-PRODUCTION-EDITOR`, `ORC-PROPOSAL-INTERACTION` | `B-S4-MANDATORY-SET` |
...
| `SMAP-RELEASE-STAGE-4` | Stage 4 | `SMAP-STAGE-4` | `S4-JRN-001` | Emit `EV-SR`, `PASS-STAGE`, the exact resulting `main`, and the exact Stage 5 input. |

`SMAP-EVALUATED-RELEASE-STAGE` is the finite dynamic selector for exactly one
of those nine release rows, keyed by the same `evaluated_stage` that selects
...
and explicit-absence boundaries to that complete Stage 2
...
evidence cannot replace fake, AgentRun-finalization, or absence proof.

## 12. Release-to-proof crosswalk

Section 12 and the companion are the complete delivery crosswalk. Each row names the proven fact, the
...
| `REL-004` | The first four stages retain their identities and are followed by the accepted Stages 5 through 9; `OWN-REL`, `OWN-STAGE`. | `DVG-01`, `DVG-02`, `DVG-03`, `DVG-04`, `DVG-05`, `DVG-06`, `DVG-07`, `DVG-08`, `DVG-09`, `DVG-10`, `DVG-11`, `DVG-13` | `EC-01`; `EV-CP/SE` | `FX-HANDOFF`, `FX-CONTRACT-R1` · contract drift and stage-boundary points · `SCH-DRIFT/NORMAL/ABSENT` · `ORC-CROSSWALK-COMPLETENESS` · `CFP-CONTRACT-DRIFT` | `B-HANDOFF` + `B-CONTRACT`; `PASS-STAGE` when the accepted nine-stage sequence and evidence dependency graph match [Define the AI-Independent Editor-First Release Baseline and Handoff Criteria](https://github.com/FrankQDWang/StoryOS/issues/62). | `BLOCK-ALL`; any unapproved stage, reordered stage, invented capability, or unresolved owner/gate row blocks. |
...
| `S2-REQ-003` | Volumes/chapters can be managed and reopened without loss. An admitted current-Chapter transition installs the authorized target base; distinct prose can be written in at least two Chapters under one Project writer and the existing ordered Journal; `OWN-WEB`, `OWN-PROTO`, `OWN-CORE`, `OWN-RET`. | `DVG-02`, `DVG-03`, `DVG-09`, `DVG-11` | `EC-02/03/04/05`; `EV-CP/IT/INT/SE` | `FX-EDITOR-IME`, `FX-REPLAY-RETENTION` · scope, replay generation/Snapshot, late-event points · `SCH-NORMAL/REPLAY/REORDER` · `ORC-SCOPE`, `ORC-REPLAY-TRUTH`, `ORC-PRODUCTION-EDITOR` · `CFP-REPLAY-AFTER-GENERATION-SNAPSHOT` | `B-EDITOR` + `B-REPLAY`; `PASS-POS`. | `BLOCK-ALL`; guessed cursor mapping, loss, or cross-scope navigation blocks. |
...
| `S2-EVD-006` | Search, writable navigation, statistics, one visible replacement, and Core typed-refusal evidence for broader change are bounded and attributable; `OWN-CTX`, `OWN-CORE`, `OWN-PROTO`. | `DVG-01`, `DVG-02`, `DVG-03`, `DVG-04`, `DVG-11` | `EC-02/05/07`; `EV-CP/IT/INT/SE` | `FX-CONTEXT-DISCLOSURE`, `FX-CORE-PROPOSAL` · scope/projection/proposal points · `SCH-SCOPE/NORMAL/LIFECYCLE` · `ORC-CONTEXT-DISCLOSURE`, `ORC-ATOMIC-AUTHORITY` · `CFP-MANIFEST-BEFORE-COMMIT` | `B-CONTEXT` + `B-CORE`; `PASS-POS` for allowed behavior and admitted typed refusal; `PASS-REFUSAL` only for pre-Admission refusal. | `BLOCK-ALL`. |
...
| `S3-JRN-001` | Seven-step Stage 3 journey—current passage/request, atomic conversation/settings and cross-Run continuation, typed provisional/complete items, current target/steering, incremental/full input and active compaction, expiry/lookup/one-successor/cancellation proof, fake full path, Inline/Block editable Proposals with stable Operations and non-overlap, input pause, edit/accept/reject, Undo Accept and fresh-Acceptance redo, exact-version comparison with coherent replacement spans, refusal/conflict/interruption, and fake-off repeat of the complete Stage 2 journey—is executable; `OWN-REL` with `OWN-PLAIN`, `OWN-CTX`, `OWN-AGENT`, `OWN-CORE`, `OWN-WEB`. Resolve `SMAP-STAGE-3` in full: its mandatory map carries the complete Stage 2 editor contract plus the fake and absence boundaries. The Stage 2 journey result is a regression output, not an input that certifies the Stage 3 map. | `SMAP-STAGE-3` | `SMAP-STAGE-3` | `SMAP-STAGE-3` | `B-S3-MANDATORY-SET`; `PASS-STAGE`. `B-FAKE` is a deterministic proof fixture, not a second Provider route. | `BLOCK-ALL`; any fake-path step, AgentRun/finalization boundary, proposal decision, unknown recovery fact, or complete Stage 2 regression item is missing, failed, stale, unrun, unavailable, unverified, or unreplayable. |
...
| `S3-EVD-002` | Host, Scope, Context, manifest, Attempt, and durable Run evidence, including atomic conversation creation/settings, exact retry identities, current instruction/target bindings, and conversation isolation for continuation. `OWN-CTX`, `OWN-AGENT`, `OWN-MODEL`, `OWN-TRUST`. `OWN-PROTO`. `OWN-PG`. | `DVG-02`, `DVG-04`, `DVG-05`, `DVG-06`, `DVG-07`, `DVG-08`, `DVG-11` | `EC-03/04/05/08`; `EV-CP/IT/INT/SE` | `FX-FAKE-MODEL`, `FX-CONTEXT-DISCLOSURE` · scope/manifest/dispatch/unknown/fence/mailbox points · `SCH-NORMAL/UNKNOWN/FENCE/SCOPE` · `ORC-CONTEXT-DISCLOSURE`, `ORC-DISPATCH-DISCLOSURE`, `ORC-OUTCOME-UNKNOWN` · `CFP-MANIFEST-AFTER-COMMIT-BEFORE-EGRESS` | `B-FAKE` + `B-CONTEXT`; `PASS-POS`/`PASS-UNKNOWN`. | `BLOCK-ALL`. |
...
| `S4-REQ-006` | Real author can write/request/inspect/edit/Accept/Reject/recover all StoryOS-owned facts; `OWN-REL`, `OWN-WEB`, `OWN-CORE`, `OWN-ADM`, `OWN-CTX`, `OWN-RET`. Resolve `SMAP-STAGE-4` so the real route is proven only after the complete Stage 3 deterministic fake/AgentRun/absence boundary and the Stage 2 editor journey remain intact; `EV-RDA` is advisory only. | `SMAP-STAGE-4` | `SMAP-STAGE-4` | `SMAP-STAGE-4` | `B-S4-REQ-006-AUTHOR-JOURNEY-SET`; `PASS-STAGE` after the S4 journey and mandatory evidence. `B-FAKE` is not a second Provider and `B-REAL-ADVISORY` cannot replace deterministic proof. | `BLOCK-ALL`; any uninspectable fact, missing decision, recovery gap, missing AgentRun finalization, hidden unsupported execution, or Provider claim substituted for StoryOS evidence blocks. |
...
| ID | What is proven · accepted owner | Gate set | Evidence | Fixture · fault points · schedule · oracle | Safe bundle · pass | Block |
| --- | --- | --- | --- | --- | --- | --- |
| `HND-001` | Planning closes against one exact `main` baseline with no unresolved contradiction or missing source/ticket acceptance coverage; `OWN-REL`, `OWN-GOV`, `OWN-DVG`. | `DVG-01`, `DVG-13` | `EC-01`; `EV-CP/SE` | `FX-CONTRACT-R1`, `FX-HANDOFF` · contract drift points · `SCH-DRIFT` · `ORC-CONTRACT`, `ORC-CROSSWALK-COMPLETENESS` · `CFP-CONTRACT-DRIFT` | `B-CONTRACT` + `B-HANDOFF`; `PASS-STAGE`. | `BLOCK-ALL`; mixed baseline, stale revision, contradiction, or missing crosswalk row blocks global planning closure. An upstream owner can close its own correction without claiming this gate. |
| `HND-002` | Each later stage consumes the accepted release and proof through its specification, then publishes only a user-approved tracer-bullet breakdown with real dependencies; `OWN-REL`, `OWN-GOV`, `OWN-STAGE`. The first Stage 1 handoff is historical. | `DVG-01`, `DVG-13` | `EC-01`; `EV-CP/SE` | `FX-CONTRACT-R1`, `FX-HANDOFF` · `CFP-CONTRACT-DRIFT` · `SCH-DRIFT` · `ORC-CONTRACT`, `ORC-CROSSWALK-COMPLETENESS` | `B-CONTRACT` + `B-HANDOFF`; `PASS-POS` for the approved planning handoff only. | `BLOCK-ALL`; missing approval, source coverage, exact body binding, real blockers, or a bounded slice blocks publication and execution. |
| `HND-003` | For exactly one currently evaluated stage, implementation evidence is complete and attributable to the exact baseline; the evidence record supplies one `evaluated_stage` and resolves only `SMAP-EVALUATED-MANDATORY-STAGE`, not an all-stage union or a release result; `OWN-REL` with that stage's accepted owners. The map is implementation evidence only and contains no stage-release disposition. | `SMAP-EVALUATED-MANDATORY-STAGE` | `SMAP-EVALUATED-MANDATORY-STAGE` | `SMAP-EVALUATED-MANDATORY-STAGE` | `SMAP-EVALUATED-MANDATORY-STAGE`; `PASS-STAGE` for the attributable mandatory evidence map only. | `BLOCK-ALL`; one selected-stage mandatory item that is failed, unrun, stale, unavailable, unverified, or unreplayable blocks, while a later stage's unrun evidence does not. HND-003 never consumes a stage-release disposition. |
| `HND-004` | Release of exactly one currently evaluated stage requires that stage's author journey and that stage's selected mandatory set both pass; only then records that stage's exact resulting `main` and the next-stage input; `OWN-REL`, `OWN-STAGE`. The evidence record supplies one `evaluated_stage`, `SMAP-EVALUATED-MANDATORY-STAGE` supplies the implementation input, and `SMAP-EVALUATED-RELEASE-STAGE` selects the corresponding ordered release branch; no all-stage union is required. | `SMAP-EVALUATED-MANDATORY-STAGE` | `SMAP-EVALUATED-MANDATORY-STAGE` | `SMAP-EVALUATED-RELEASE-STAGE` | `SMAP-EVALUATED-RELEASE-STAGE`; only after its mandatory map and author journey pass does it emit `EV-SR`, `PASS-STAGE`, the exact resulting `main`, and the next-stage input. | `BLOCK-ALL`; passing gates without the selected stage journey, or the journey without the selected stage mandatory pass, is not release, and a later stage's unrun evidence does not block this stage. |
| `HND-005` | After the first four stages, controlled-cloud evidence for the exact selected released local stage separately proves identity/security/recovery/cache/same-release/upgrade; `OWN-REL`, `OWN-PG`, `OWN-PROTO`, `OWN-TRUST`. | `DVG-01`, `DVG-02`, `DVG-04`, `DVG-05`, `DVG-07`, `DVG-09`, `DVG-10`, `DVG-11` | `EC-04/05/08`; `EV-CP/PRD/RDA/CCD` | `FX-RESTORE-LIFECYCLE`, `FX-CONTEXT-DISCLOSURE`, `FX-HANDOFF` · contract/scope/manifest/restore/lifecycle points · `SCH-DRIFT/SCOPE/RESTORE/LIFECYCLE/UNKNOWN` · `ORC-CONTRACT`, `ORC-SCOPE`, `ORC-RESTORE-LIFECYCLE` · `CFP-RESTORE-STAGING` | `B-RESTORE` + `B-CONTEXT` + `B-HANDOFF`; `PASS-CLOUD` only when later deployment owner adopts and completes it. | `BLOCK-ALL` for the later cloud handoff; it cannot block or upgrade local release by implication, require all later features first, or certify unselected later features. |
| `HND-006` | Release requirements flow from `OWN-REL` to `OWN-DVG`, the relevant stage specification, and its approved child graph. The original first Stage 1 handoff stays closed; `OWN-GOV` enforces one claimed implementation ticket at a time. | `DVG-13` | `EC-01`; `EV-CP/SE` | `FX-HANDOFF` · `CFP-CONTRACT-DRIFT` · `SCH-DRIFT`, `SCH-NORMAL` · `ORC-CROSSWALK-COMPLETENESS` | `B-HANDOFF`; `PASS-POS`. | `BLOCK-ALL`; an unapproved graph, missing acceptance owner, body/native-edge mismatch, cycle, or parallel execution blocks the handoff. Serial priority alone does not create a technical dependency. |
...
- every accepted `REL-*`, `HND-*`, and Stage 1 through Stage 9 Requirement,
  journey, and evidence identifier occurs in the combined crosswalk exactly once and resolves to existing gate, owner, evidence,
  fixture, fault-point, schedule, oracle, bundle, pass, and block definitions;
- every `DVG-*`, `CFP-*`, `FX-*`, `SCH-*`, `B-*`, `EC-*`, `EV-*`, and `ORC-*`
  reference resolves to this document or the companion catalogue;
...
- no `.reference/**` file changes or dependency enters the proof input, and
  changes stay within the two proof documents; and
...
wide reference inventory and explicit propagation analysis. The remaining
non-Eval gate identifiers keep their existing meanings. Historical Stage 1
selections and exclusions remain unchanged.
...
Historical implementation and prototype evidence retains its original claim
...
The existing Stage 3, 4, 5, and 7 specifications and child contracts still
require their separate handoff. Stage 3 and later implementation remains
EXECUTION HOLD; this proof decision releases no product stage.
