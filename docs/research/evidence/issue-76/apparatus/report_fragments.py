"""Render report-contract fragments from derived Issue #76 evidence."""
...
def find_distribution(
    ...
    **identity: object,
    ...
    if len(matches) != 1:
        rendered = "/".join(str(value) for value in identity.values())
        raise ValueError(f"expected one distribution for {rendered}")
    return matches[0]
...
def build_report_fragments(
    *,
    ...
    browser_distributions: list[dict[str, object]],
    operation_distributions: list[dict[str, object]],
    families: tuple[str, ...],
    slopes: dict[tuple[str, str], float],
    shape_probe_slopes: dict[tuple[str, str], float],
    models: list[dict[str, object]],
    journal: dict[str, object],
    compaction_rows: list[dict[str, object]],
    restore_rows: list[dict[str, object]],
    critical_claims: dict[str, object],
) -> dict[str, str]:
    """Build the exact raw-derived fragments audited in the frozen report."""
    ...
    report_fragments.update(
        {
            ...
            "environment_browser_heap": (
                f"a {heap_limit_gib:.1f} GiB JavaScript heap limit"
            ),
            ...
            "workload_novel": (
                ...
                f"{novel_total_mb:.1f} MB; "
                f"{workload['browser']['chapter_switch_samples']} switches and "
                f"{workload['browser']['cold_open_samples']} page reload/cold opens"
            ),
            "workload_postgres_growth": (
                f"{record_counts_label} records per Revision, Receipt, Activity "
                "Event, and Application Wire Record family"
            ),
            "workload_replay": (
                f"{workload['postgres']['replay_events_without_checkpoint'] // 1000}K "
                f"Event scan versus {replay_after_k}K Event tail after one checkpoint"
            ),
            "workload_compaction": (
                f"{workload['postgres']['record_counts'][-1] // 1000}K "
                f"{compact_payload_kib}-KiB Event payloads; delete "
    ...
    report_fragments.update(
        {
            "journal_serialized_bytes": (
                "- attributable serialized journal records: "
                f"`{int(journal['logical_serialized_bytes']):,} bytes`;"
            ),
            "journal_naive_bytes": (
                "- hypothetical before-plus-after full-document copies: "
            ...
            "journal_amplification": (
                ...
                "to that deliberately naive comparator."
            ),
    ...
    report_fragments["postgres_shape_ratio"] = (
        f"`{shape_high / shape_low:.2f}×` span"
    )
    ...
    report_fragments["recommendation_storage_slope"] = (
        ...
        "B/command`; 4-KiB sensitivity implies up to "
