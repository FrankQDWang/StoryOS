...
This decision does not change `web_client_contract_revision`, IndexedDB meaning, or the Apply Author Edit command schema. One Author Edit Unit may apply an ordered list of the existing primitives so a contiguous replacement across Blocks settles atomically.
