...
UniqueID declares the paragraph and heading `id` attributes. The Protected Web Client adapter allocates a candidate StoryOS UUID v7 for each new author-created Block. The adapter writes that candidate into the document and sets UniqueID `generateID` to allocate a StoryOS UUID v7. The adapter sets UniqueID `updateDocument` to `false`, so UniqueID does not mint or repair an id during hydration. Core owns identity validation, authoritative settlement, and the stable Manuscript Block identity contract.
...
- Allowing autonomous UniqueID minting or repair during hydration, or UniqueID as the identity owner, was rejected because Core owns identity validation, authoritative settlement, and the stable Manuscript Block identity contract.
