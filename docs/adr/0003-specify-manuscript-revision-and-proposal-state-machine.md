...
- PostgreSQL schema, transaction isolation, durability, separately stored payload policy, and migration remain deferred to [Specify the PostgreSQL Project Storage, Isolation, and Migration Contract](https://github.com/FrankQDWang/StoryOS/issues/56), but that storage must implement the atomic logical boundary and Project Isolation defined here. This consequence supersedes the earlier SQLite assumption without changing the state machine.
