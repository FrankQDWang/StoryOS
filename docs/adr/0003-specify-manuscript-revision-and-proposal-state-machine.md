...
`ApplyAuthorEdit` is the single ownership-classifying entry point: Core recomputes current Heads, Anchors, reservations, and ownership, then atomically settles the whole command as authoritative, Proposal-revised, refused-to-Draft, conflicted, or no-effect. One completed semantic editor intent is the conservative commit boundary. Bounded idle coalescing is permitted only while Scope, chapter, target, ownership, Heads, writer generation, Admission, editor contract, and undo-group bindings remain exactly equal and a current limit profile supplies hard bounds. The client never selects an authoritative route.

Proposals retain four orthogonal state axes, block-relative versioned Anchors, fail-closed target conflicts, exact-head Acceptance and lifecycle decisions, and typed newest-first compensation. Refused Edit Draft and Recovery Draft are the editor flow's only Draft Artifacts; Proposal Conflict and Proposal Recovery Conflict are conditions on preserved Proposal surfaces. Admission recovery treats post-admission uncertainty as `outcome_unknown`, reconciles from durable Receipt/idempotency facts, invokes automatically only the same unexpired fully matching direct edit, and requires a fresh author confirmation for explicit, expired, changed, or unrecoverable commands. Missing response never proves non-commit.
