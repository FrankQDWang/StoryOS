...
export async function getAgentRun({ projectId, runId, ...options } = {}) {
  ...
  return queryJson({ ...options, path: `/api/v1/projects/${encodeURIComponent(projectId)}/agent-runs/${encodeURIComponent(runId)}` });
}
...
export async function digestDeleteVolume(request, cryptoImpl = globalThis.crypto) {
  ...
  const bytes = new TextEncoder().encode(JSON.stringify(canonical));
  const digest = new Uint8Array(await cryptoImpl.subtle.digest("SHA-256", bytes));
  return { algorithm: "sha256", profile: "storyos.command.deleteVolume.jcs.v1", value_hex_lowercase: [...digest].map((byte) => byte.toString(16).padStart(2, "0")).join("") };
}
...
export async function deleteVolume({ projectId, volumeId, request, idempotencyKey, antiForgery, ...options } = {}) {
  ...
  return commandJson({ ...options, method: "DELETE", path: `/api/v1/projects/${encodeURIComponent(projectId)}/volumes/${encodeURIComponent(volumeId)}`, body: request, commandHeaders: { "idempotency-key": idempotencyKey, "x-storyos-anti-forgery": antiForgery } });
}
