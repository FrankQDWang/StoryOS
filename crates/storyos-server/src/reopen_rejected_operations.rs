...
use storyos_application::{
    ...
    ProjectCommandChallengeBinding, ReopenRejectedOperationsCommand, ReopenRejectedOperationsError,
    ReopenRejectedOperationsSettlementEffect,
};
...
pub(super) async fn reopen_rejected_operations(
    ...
    let body = serde_json::from_slice::<contracts::ReopenRejectedOperationsRequest>(&bytes)
    ...
    if body.command_schema != contracts::REOPEN_REJECTED_OPERATIONS_REQUEST_SCHEMA_ID
        ...
        || input.selected_rejected_operation_ids.len() != 1
        || input.rejection_event_refs.len() != 1
        || input.expected_target_revisions.len() != 1
    {
        return Err(invalid_request());
    }
    let selected_rejected_operation_id = input.selected_rejected_operation_ids[0].clone();
    let rejection_event_id = input.rejection_event_refs[0].clone();
    ...
    let canonical_command_digest = format!(
        "sha256:{}:{digest_hex}",
        contracts::REOPEN_REJECTED_OPERATIONS_DIGEST_PROFILE
    );
    let store = project_reader(&state).await?;
    let command = ReopenRejectedOperationsCommand {
        ...
        challenge_binding: ProjectCommandChallengeBinding {
            ...
            method: contracts::REOPEN_REJECTED_OPERATIONS_METHOD.to_owned(),
            route_template: contracts::REOPEN_REJECTED_OPERATIONS_PATH.to_owned(),
            command_schema: body.command_schema.clone(),
            command_kind: "reopenRejectedOperations".to_owned(),
    ...
    let settlement = storyos_application::reopen_rejected_operations(&store, &command)
        .await
...
fn reopen_response(
    ...
    digest_hex: &str,
    settlement: storyos_application::ReopenRejectedOperationsSettlement,
) -> Result<Json<contracts::ReopenRejectedOperationsResponse>, ApiError> {
    ...
    let resulting_proposal_revision_id = match &settlement.effect {
        ReopenRejectedOperationsSettlementEffect::Resolved {
        ...
        | ReopenRejectedOperationsSettlementEffect::Refused { .. } => {
            command.proposal_revision_id.clone()
        }
    };
    let (result, effect) = match settlement.effect {
        ReopenRejectedOperationsSettlementEffect::Resolved {
            ...
            contracts::ReopenRejectedOperationsEffect::Resolved {
                ...
                rejection_event_refs: vec![rejection_event_id],
        ...
        ReopenRejectedOperationsSettlementEffect::Conflicted { reason } => (
            ...
            contracts::ReopenRejectedOperationsEffect::Conflicted {
                reason: match reason {
                    storyos_core::ReopenRejectedOperationsConflict::ChangedHead => {
        ...
        ReopenRejectedOperationsSettlementEffect::Refused { reason } => (
            ...
            contracts::ReopenRejectedOperationsEffect::Refused {
                reason: match reason {
                    storyos_core::ReopenRejectedOperationsRefusal::WrongScope => {
                        contracts::ReopenRejectedOperationsRefusalReason::WrongScope
                    }
                    storyos_core::ReopenRejectedOperationsRefusal::WrongAdmission => {
                    ...
                    storyos_core::ReopenRejectedOperationsRefusal::NotEligible => {
                    ...
                    storyos_core::ReopenRejectedOperationsRefusal::UnavailableProof => {
    ...
    Ok(Json(contracts::ReopenRejectedOperationsResponse {
        ...
        receipt: contracts::ReopenReceipt {
            ...
            resulting_proposal_revision_id,
            selected_rejected_operation_ids: vec![command.selected_rejected_operation_id.clone()],
            rejection_event_refs: vec![command.rejection_event_id.clone()],
...
fn reopen_error(error: ReopenRejectedOperationsError) -> ApiError {
    match error {
        ReopenRejectedOperationsError::BindingConflict => problem(
            StatusCode::CONFLICT,
            "idempotency_binding_conflict",
            "The Reopen binding conflicts.",
        ),
        ReopenRejectedOperationsError::HistoricalAcknowledgementUnavailable => problem(
            StatusCode::CONFLICT,
        ...
        ReopenRejectedOperationsError::InvalidChallenge => problem(
            StatusCode::UNPROCESSABLE_ENTITY,
            "challenge_invalid",
            "The Reopen challenge is invalid.",
        ),
        ReopenRejectedOperationsError::MissingProject => resource_unavailable(),
        ReopenRejectedOperationsError::Unavailable(_) => problem(
