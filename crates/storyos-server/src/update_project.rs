...
pub(super) async fn update_project(
    ...
    hold_first_acknowledgement_if_requested(idempotency_key).await;
...
async fn hold_first_acknowledgement_if_requested(idempotency_key: &str) {
...
fn update_project_error(error: UpdateProjectError) -> ApiError {
    match error {
        ...
        UpdateProjectError::HistoricalAcknowledgementUnavailable => problem(
            StatusCode::CONFLICT,
            "historical_acknowledgement_unavailable",
            "The original Update Project acknowledgement cannot be recovered. Refresh to inspect the current Project.",
        ),
