...
impl ArchiveExportWorkStore for PostgresProjectReader {
    async fn claim_next_archive_export(
        &self,
    ...
    let Some(operation) = client
        .query_opt(
            "SELECT operation.settled_result,
                    EXISTS (
                      SELECT 1 FROM storyos.project_export_manifests AS output
                       WHERE output.owner_user_id = operation.owner_user_id
                         ...
                         AND output.immutable_root IS NOT NULL
                    )
               FROM storyos.project_export_operations AS operation
        ...
        return Err(CompleteArchiveExportError::StaleFence);
    };
    if operation.get::<_, Option<String>>(0).is_some() || operation.get::<_, bool>(1) {
        return Ok(CompleteArchiveExport::AlreadySettled);
    }
...
async fn persist_ready_export(
    ...
    let payload = serde_json::json!({
        ...
        "archive_profile": PROJECT_EXPORT_ARCHIVE_PROFILE,
        "archive_path_profile": PROJECT_EXPORT_ARCHIVE_PATH_PROFILE,
    })
    .to_string();
    client
        .execute(
    ...
    settle_export_operation(
        client,
        claim,
        &receipt_id,
        /*settled_result*/ Some("ready"),
    )
    .await?;
    Ok(CompleteArchiveExport::SettledReady)
}
...
async fn persist_failed_export(
    ...
    settle_export_operation(
        ...
        claim,
        &receipt_id,
        /*settled_result*/ Some("failed"),
    )
    .await?;
    Ok(CompleteArchiveExport::SettledFailed)
}
...
async fn insert_export_receipt(
    ...
    result_kind: &str,
    result_payload: &str,
) -> Result<String, CompleteArchiveExportError> {
...
) -> Result<(), CompleteArchiveExportError> {
    let updated = client
        .execute(
            "UPDATE storyos.project_export_operations
...
fn complete_database_error(error: tokio_postgres::Error) -> CompleteArchiveExportError {
    CompleteArchiveExportError::unavailable(error)
}

fn complete_read_error(error: ProjectReadError) -> CompleteArchiveExportError {
    CompleteArchiveExportError::unavailable(error)
}
