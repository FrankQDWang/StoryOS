...
ALTER TABLE storyos.manuscript_objects
  DROP CONSTRAINT manuscript_objects_object_kind_check;

ALTER TABLE storyos.manuscript_objects
  ADD CONSTRAINT manuscript_objects_object_kind_check
  CHECK (object_kind IN ('chapter', 'volume'));

ALTER TABLE storyos.manuscript_objects
  ADD COLUMN tree_order bigint NOT NULL DEFAULT 1
    CHECK (tree_order >= 1);

CREATE UNIQUE INDEX manuscript_objects_volume_tree_order
  ON storyos.manuscript_objects (owner_user_id, project_id, tree_order)
  WHERE object_kind = 'volume';

GRANT INSERT ON storyos.manuscript_objects TO storyos_runtime;
