...
ALTER TABLE storyos.agent_runs
  ADD CONSTRAINT agent_runs_predecessor_fk
  FOREIGN KEY (owner_user_id, project_id, predecessor_run_id)
  REFERENCES storyos.agent_runs (owner_user_id, project_id, run_id);

ALTER TABLE storyos.proposal_generations
  ADD COLUMN run_id uuid;
ALTER TABLE storyos.proposal_generations
  ADD CONSTRAINT proposal_generations_run_fk
  FOREIGN KEY (owner_user_id, project_id, run_id)
  REFERENCES storyos.agent_runs (owner_user_id, project_id, run_id);

CREATE TABLE storyos.proposal_generation_heads (
...
SELECT DISTINCT ON (owner_user_id, project_id, proposal_id)
       owner_user_id, project_id, proposal_id, generation_id
  FROM storyos.proposal_generations
 ORDER BY owner_user_id, project_id, proposal_id, generation_id;

CREATE TABLE storyos.proposal_generation_transitions (
  owner_user_id uuid NOT NULL,
  project_id uuid NOT NULL,
  transition_id uuid NOT NULL,
  proposal_id uuid NOT NULL,
  proposal_revision_id uuid NOT NULL,
  prior_generation_id uuid NOT NULL,
  resulting_generation_id uuid NOT NULL,
  prior_generation_state text NOT NULL
    CHECK (prior_generation_state IN ('generating', 'ready_partial', 'ready')),
  resulting_generation_state text NOT NULL
    CHECK (resulting_generation_state IN ('generating', 'ready_partial', 'ready')),
  prior_run_id uuid NOT NULL,
  resulting_run_id uuid NOT NULL,
  preserved_validation text NOT NULL,
  preserved_closure text NOT NULL,
  preserved_operation_resolution text NOT NULL,
  receipt_id uuid NOT NULL,
  author_action_sequence numeric(20, 0) NOT NULL
    CHECK (author_action_sequence BETWEEN 1 AND 18446744073709551615),
  PRIMARY KEY (owner_user_id, project_id, transition_id),
  ...
  FOREIGN KEY (owner_user_id, project_id, prior_generation_id)
    REFERENCES storyos.proposal_generations
      (owner_user_id, project_id, generation_id) MATCH FULL,
  FOREIGN KEY (owner_user_id, project_id, resulting_generation_id)
    REFERENCES storyos.proposal_generations
      (owner_user_id, project_id, generation_id) MATCH FULL,
  FOREIGN KEY (owner_user_id, project_id, prior_run_id)
    REFERENCES storyos.agent_runs (owner_user_id, project_id, run_id) MATCH FULL,
  FOREIGN KEY (owner_user_id, project_id, resulting_run_id)
    REFERENCES storyos.agent_runs (owner_user_id, project_id, run_id) MATCH FULL,
...
ALTER TABLE storyos.proposal_generation_heads ENABLE ROW LEVEL SECURITY;
ALTER TABLE storyos.proposal_generation_heads FORCE ROW LEVEL SECURITY;
ALTER TABLE storyos.proposal_generation_transitions ENABLE ROW LEVEL SECURITY;
ALTER TABLE storyos.proposal_generation_transitions FORCE ROW LEVEL SECURITY;

CREATE POLICY proposal_generation_heads_exact_scope ON storyos.proposal_generation_heads USING (
...
CREATE POLICY proposal_generation_transitions_exact_scope
  ON storyos.proposal_generation_transitions USING (
  owner_user_id = NULLIF(current_setting('storyos.owner_user_id', true), '')::uuid
  AND project_id = NULLIF(current_setting('storyos.project_id', true), '')::uuid
) WITH CHECK (
  owner_user_id = NULLIF(current_setting('storyos.owner_user_id', true), '')::uuid
...
GRANT SELECT, INSERT, UPDATE ON storyos.proposal_generation_heads TO storyos_runtime;
GRANT SELECT, INSERT ON storyos.proposal_generation_transitions TO storyos_runtime;
