...
ALTER TABLE storyos.projects
  DROP CONSTRAINT projects_lifecycle_state_check;

ALTER TABLE storyos.projects
  ADD CONSTRAINT projects_lifecycle_state_check
  CHECK (lifecycle_state IN ('active', 'archived'));

CREATE TABLE storyos.project_archival_decisions (
  owner_user_id uuid NOT NULL,
  project_id uuid NOT NULL,
  archival_decision_id uuid NOT NULL,
  receipt_id uuid NOT NULL,
  prior_lifecycle_state text NOT NULL CHECK (prior_lifecycle_state = 'active'),
  resulting_lifecycle_state text NOT NULL CHECK (resulting_lifecycle_state = 'archived'),
  project_revision bigint NOT NULL CHECK (project_revision >= 1),
  created_at timestamptz NOT NULL DEFAULT clock_timestamp(),
  PRIMARY KEY (owner_user_id, project_id, archival_decision_id),
...
ALTER TABLE storyos.project_archival_decisions ENABLE ROW LEVEL SECURITY;
ALTER TABLE storyos.project_archival_decisions FORCE ROW LEVEL SECURITY;
CREATE POLICY project_archival_decisions_exact_scope
  ON storyos.project_archival_decisions USING (
...
GRANT SELECT, INSERT ON storyos.project_archival_decisions TO storyos_runtime;
...
FOR EACH ROW
WHEN (NEW.command_kind NOT IN ('createProject', 'updateProject', 'archiveProject'))
...
ALTER TABLE storyos.domain_receipts
  ADD CONSTRAINT domain_receipts_common_shape CHECK ((
    ...
    AND (
      (command_kind NOT IN ('createProject', 'updateProject', 'archiveProject')
      ...
      OR (command_kind IN ('createProject', 'updateProject', 'archiveProject')
...
CREATE CONSTRAINT TRIGGER project_archival_decisions_receipt_relation_complete
AFTER INSERT OR UPDATE OR DELETE ON storyos.project_archival_decisions
