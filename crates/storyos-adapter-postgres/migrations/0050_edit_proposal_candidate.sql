...
GRANT UPDATE ON storyos.proposal_heads TO storyos_runtime;

ALTER TABLE storyos.proposal_revisions
  ADD COLUMN parent_revision_id uuid;
ALTER TABLE storyos.proposal_revisions
  ADD CONSTRAINT proposal_revisions_parent_fk
  FOREIGN KEY (owner_user_id, project_id, proposal_id, parent_revision_id)
    REFERENCES storyos.proposal_revisions
      (owner_user_id, project_id, proposal_id, revision_id) MATCH SIMPLE;

ALTER TABLE storyos.author_command_admissions
  DROP CONSTRAINT author_command_admissions_observed_ownership_partition_check;
ALTER TABLE storyos.author_command_admissions
  ADD CONSTRAINT author_command_admissions_observed_ownership_partition_check
  CHECK (
    observed_ownership_partition IS NULL
    OR observed_ownership_partition IN ('authoritative', 'mixed')
  );
...
ALTER TABLE storyos.domain_receipts
  ...
  CHECK (result_kind IN (
    'authoritative_applied', 'proposal_revised', 'no_effect', 'conflicted', 'refused'
  ));
...
ALTER TABLE storyos.author_action_entries
  ...
  CHECK (receipt_result_kind IN ('authoritative_applied', 'proposal_revised'));
