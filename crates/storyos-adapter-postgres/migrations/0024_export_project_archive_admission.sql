...
CREATE TABLE storyos.project_export_manifests (
  ...
  archive_profile text NOT NULL CHECK (archive_profile = 'storyos.project-export.v1'),
  archive_path_profile text NOT NULL
    CHECK (archive_path_profile = 'storyos.archive-path.utf8-nfc-unicode-16.0.0.v1'),
  immutable_root text
    CHECK (immutable_root IS NULL OR immutable_root ~ '^sha256:[0-9a-f]{64}$'),
...
ALTER TABLE storyos.project_export_manifests ENABLE ROW LEVEL SECURITY;
ALTER TABLE storyos.project_export_manifests FORCE ROW LEVEL SECURITY;
CREATE POLICY project_export_manifests_exact_scope
  ON storyos.project_export_manifests USING (
...
GRANT SELECT, INSERT ON storyos.project_export_manifests TO storyos_runtime;
...
ALTER TABLE storyos.author_command_admissions
  ADD CONSTRAINT author_command_admissions_command_shape CHECK ((
    ...
    OR (command_kind IN ('deleteChapter', 'deleteVolume', 'exportHumanReadableManuscript', 'exportProjectArchive')
      AND action_class = 'explicit_project_command'
      AND editor_session_id IS NULL
      AND writer_generation IS NULL
      AND chapter_object_id IS NULL
