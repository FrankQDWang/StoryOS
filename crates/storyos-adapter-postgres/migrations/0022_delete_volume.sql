...
CREATE TABLE storyos.volume_removal_decisions (
  owner_user_id uuid NOT NULL,
  project_id uuid NOT NULL,
  volume_removal_decision_id uuid NOT NULL,
  receipt_id uuid NOT NULL,
  volume_id uuid NOT NULL,
  tree_revision bigint NOT NULL CHECK (tree_revision >= 1),
  created_at timestamptz NOT NULL DEFAULT clock_timestamp(),
  PRIMARY KEY (owner_user_id, project_id, volume_removal_decision_id),
  UNIQUE (owner_user_id, project_id, receipt_id),
  UNIQUE (owner_user_id, project_id, volume_id),
  ...
  FOREIGN KEY (owner_user_id, project_id, volume_id)
    REFERENCES storyos.manuscript_objects(owner_user_id, project_id, manuscript_object_id) MATCH FULL
);

ALTER TABLE storyos.volume_removal_decisions ENABLE ROW LEVEL SECURITY;
ALTER TABLE storyos.volume_removal_decisions FORCE ROW LEVEL SECURITY;
CREATE POLICY volume_removal_decisions_exact_scope
  ON storyos.volume_removal_decisions USING (
...
GRANT SELECT, INSERT ON storyos.volume_removal_decisions TO storyos_runtime;
...
ALTER TABLE storyos.author_command_admissions
  ADD CONSTRAINT author_command_admissions_command_shape CHECK ((
    ...
    OR (command_kind IN ('deleteChapter', 'deleteVolume')
      AND action_class = 'explicit_project_command'
      AND editor_session_id IS NULL
      AND writer_generation IS NULL
      AND chapter_object_id IS NULL
