...
CREATE TABLE storyos.proposal_rejection_receipts (
  owner_user_id uuid NOT NULL,
  project_id uuid NOT NULL,
  rejection_receipt_id uuid NOT NULL,
  proposal_id uuid NOT NULL,
  proposal_revision_id uuid NOT NULL,
  selected_operation_id uuid NOT NULL,
  result text NOT NULL
    CHECK (result IN ('proposal_operations_resolved', 'conflicted', 'refused')),
  rejection_reason text
    CHECK ((result = 'proposal_operations_resolved') = (rejection_reason = 'author_declined')),
  rejection_note text,
  PRIMARY KEY (owner_user_id, project_id, rejection_receipt_id),
  FOREIGN KEY (owner_user_id, project_id, rejection_receipt_id)
...
ALTER TABLE storyos.proposal_rejection_receipts ENABLE ROW LEVEL SECURITY;
ALTER TABLE storyos.proposal_rejection_receipts FORCE ROW LEVEL SECURITY;
CREATE POLICY proposal_rejection_receipts_exact_scope ON storyos.proposal_rejection_receipts USING (
...
GRANT SELECT, INSERT ON storyos.proposal_rejection_receipts TO storyos_runtime;

CREATE TABLE storyos.proposal_operation_resolutions (
  owner_user_id uuid NOT NULL,
  project_id uuid NOT NULL,
  ...
  prior_resolution text NOT NULL CHECK (prior_resolution = 'pending'),
  resulting_resolution text NOT NULL CHECK (resulting_resolution = 'rejected'),
  rejection_receipt_id uuid NOT NULL,
  author_action_sequence numeric(20, 0) NOT NULL
    CHECK (author_action_sequence BETWEEN 1 AND 18446744073709551615),
  PRIMARY KEY (owner_user_id, project_id, resolution_event_id),
  UNIQUE (owner_user_id, project_id, rejection_receipt_id, operation_id),
  FOREIGN KEY (owner_user_id, project_id, rejection_receipt_id)
    REFERENCES storyos.proposal_rejection_receipts
      (owner_user_id, project_id, rejection_receipt_id) MATCH FULL,
...
ALTER TABLE storyos.proposal_operation_resolutions ENABLE ROW LEVEL SECURITY;
ALTER TABLE storyos.proposal_operation_resolutions FORCE ROW LEVEL SECURITY;
CREATE POLICY proposal_operation_resolutions_exact_scope ON storyos.proposal_operation_resolutions USING (
  owner_user_id = NULLIF(current_setting('storyos.owner_user_id', true), '')::uuid
  AND project_id = NULLIF(current_setting('storyos.project_id', true), '')::uuid
) WITH CHECK (
  owner_user_id = NULLIF(current_setting('storyos.owner_user_id', true), '')::uuid
...
GRANT SELECT, INSERT ON storyos.proposal_operation_resolutions TO storyos_runtime;
