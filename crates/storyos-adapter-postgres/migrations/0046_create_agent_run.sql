...
CREATE TABLE storyos.project_agents (
  owner_user_id uuid NOT NULL,
  project_id uuid NOT NULL,
  project_agent_id uuid NOT NULL,
  PRIMARY KEY (owner_user_id, project_id, project_agent_id),
...
CREATE TABLE storyos.project_conversations (
  owner_user_id uuid NOT NULL,
  project_id uuid NOT NULL,
  conversation_id uuid NOT NULL,
  project_agent_id uuid NOT NULL,
  created_receipt_id uuid NOT NULL,
  PRIMARY KEY (owner_user_id, project_id, conversation_id),
  FOREIGN KEY (owner_user_id, project_id, project_agent_id)
    REFERENCES storyos.project_agents(owner_user_id, project_id, project_agent_id)
    MATCH FULL,
  FOREIGN KEY (owner_user_id, project_id, created_receipt_id)
    REFERENCES storyos.domain_receipts(owner_user_id, project_id, receipt_id) MATCH FULL
);
CREATE INDEX project_conversations_agent_idx
  ON storyos.project_conversations (owner_user_id, project_id, project_agent_id);
CREATE TABLE storyos.conversation_memory_settings (
  owner_user_id uuid NOT NULL,
  project_id uuid NOT NULL,
  conversation_id uuid NOT NULL,
  memory_settings_revision uuid NOT NULL,
  use_enabled boolean NOT NULL,
  contribution_enabled boolean NOT NULL,
  receipt_id uuid NOT NULL,
  PRIMARY KEY (
    owner_user_id, project_id, conversation_id, memory_settings_revision
  ),
  UNIQUE (owner_user_id, project_id, conversation_id),
...
CREATE TABLE storyos.agent_runs (
  owner_user_id uuid NOT NULL,
  project_id uuid NOT NULL,
  ...
  author_message text NOT NULL
    CHECK (char_length(author_message) BETWEEN 1 AND 8000),
  status text NOT NULL CHECK (status IN ('queued')),
  receipt_id uuid NOT NULL,
  PRIMARY KEY (owner_user_id, project_id, run_id),
  FOREIGN KEY (
    owner_user_id, project_id, conversation_id, memory_settings_revision
  )
    REFERENCES storyos.conversation_memory_settings(
      owner_user_id, project_id, conversation_id, memory_settings_revision
    ) MATCH FULL,
...
ALTER TABLE storyos.project_agents ENABLE ROW LEVEL SECURITY;
ALTER TABLE storyos.project_agents FORCE ROW LEVEL SECURITY;
ALTER TABLE storyos.project_conversations ENABLE ROW LEVEL SECURITY;
ALTER TABLE storyos.project_conversations FORCE ROW LEVEL SECURITY;
ALTER TABLE storyos.conversation_memory_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE storyos.conversation_memory_settings FORCE ROW LEVEL SECURITY;
ALTER TABLE storyos.agent_runs ENABLE ROW LEVEL SECURITY;
ALTER TABLE storyos.agent_runs FORCE ROW LEVEL SECURITY;

CREATE POLICY project_agents_exact_scope
  ON storyos.project_agents USING (
...
CREATE POLICY project_conversations_exact_scope
  ON storyos.project_conversations USING (
    owner_user_id = current_setting('storyos.owner_user_id')::uuid
    AND project_id = current_setting('storyos.project_id')::uuid
  ) WITH CHECK (
    owner_user_id = current_setting('storyos.owner_user_id')::uuid
...
CREATE POLICY conversation_memory_settings_exact_scope
  ON storyos.conversation_memory_settings USING (
    owner_user_id = current_setting('storyos.owner_user_id')::uuid
    AND project_id = current_setting('storyos.project_id')::uuid
  ) WITH CHECK (
    owner_user_id = current_setting('storyos.owner_user_id')::uuid
...
GRANT SELECT, INSERT ON storyos.project_agents,
  storyos.project_conversations,
  storyos.conversation_memory_settings,
  storyos.agent_runs TO storyos_runtime;
...
ALTER TABLE storyos.project_activity_event_payloads
  ...
  CHECK (event_kind IN (
    ...
    'project_assistance_updated', 'agent_run_created'
  ));
ALTER TABLE storyos.project_activity_event_payloads
...
ALTER TABLE storyos.project_activity_event_payloads
  ADD CONSTRAINT project_activity_event_payloads_shape CHECK ((
    ...
    AND (
      ...
      OR (event_kind = 'agent_run_created'
        AND receipt_result_kind = 'authoritative_applied'
        AND payload->>'kind' = 'agent_run_created'
        AND jsonb_typeof(payload->'project_agent_id') = 'string'
        AND jsonb_typeof(payload->'conversation_id') = 'string'
        AND jsonb_typeof(payload->'memory_settings_revision') = 'string'
        AND jsonb_typeof(payload->'run_id') = 'string'
        AND payload - 'kind' - 'project_agent_id' - 'conversation_id'
          - 'memory_settings_revision' - 'run_id' = '{}'::jsonb)
    )
  ) IS TRUE);
