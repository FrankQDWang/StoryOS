...
CREATE TABLE storyos.human_readable_manuscript_exports (
  ...
  export_profile text NOT NULL CHECK (export_profile = 'storyos.readable-export.utf8-lf.v1'),
  manuscript_utf8 text NOT NULL,
...
ALTER TABLE storyos.human_readable_manuscript_exports ENABLE ROW LEVEL SECURITY;
ALTER TABLE storyos.human_readable_manuscript_exports FORCE ROW LEVEL SECURITY;
CREATE POLICY human_readable_manuscript_exports_exact_scope
  ON storyos.human_readable_manuscript_exports USING (
...
GRANT SELECT, INSERT ON storyos.human_readable_manuscript_exports TO storyos_runtime;
...
ALTER TABLE storyos.author_command_admissions
  ADD CONSTRAINT author_command_admissions_command_shape CHECK ((
    ...
    OR (command_kind IN ('deleteChapter', 'deleteVolume', 'exportHumanReadableManuscript')
      AND action_class = 'explicit_project_command'
      AND editor_session_id IS NULL
      AND writer_generation IS NULL
      AND chapter_object_id IS NULL
