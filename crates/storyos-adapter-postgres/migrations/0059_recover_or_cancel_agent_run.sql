...
ALTER TABLE storyos.agent_runs
  ...
  CHECK (status IN (
    'queued', 'claimed', 'waiting', 'paused', 'completed', 'refused', 'cancelled'
  ));

DROP INDEX storyos.agent_runs_one_live_conversation;
CREATE UNIQUE INDEX agent_runs_one_live_conversation
  ON storyos.agent_runs (owner_user_id, project_id, conversation_id)
  WHERE status IN ('queued', 'claimed', 'waiting', 'paused');
...
WHEN (NEW.command_kind NOT IN (
  ...
  'updateProjectAssistance', 'createAgentRun', 'pauseAgentRun', 'cancelAgentRun'
))
...
ALTER TABLE storyos.project_activity_event_payloads
  ...
  CHECK (event_kind IN (
    ...
    'agent_run_paused', 'agent_run_cancelled'
  ));
ALTER TABLE storyos.project_activity_event_payloads
...
ALTER TABLE storyos.project_activity_event_payloads
  ADD CONSTRAINT project_activity_event_payloads_shape CHECK ((
    ...
    AND (
      ...
      OR (event_kind = 'agent_run_paused'
        AND receipt_result_kind = 'authoritative_applied'
        AND payload->>'kind' = 'agent_run_paused'
        AND jsonb_typeof(payload->'run_id') = 'string'
        AND jsonb_typeof(payload->'fence_generation') = 'string'
        AND payload - 'kind' - 'run_id' - 'fence_generation' = '{}'::jsonb)
      OR (event_kind = 'agent_run_cancelled'
        AND receipt_result_kind = 'authoritative_applied'
        AND payload->>'kind' = 'agent_run_cancelled'
        AND jsonb_typeof(payload->'run_id') = 'string'
        AND jsonb_typeof(payload->'fence_generation') = 'string'
        AND payload - 'kind' - 'run_id' - 'fence_generation' = '{}'::jsonb)
    )
