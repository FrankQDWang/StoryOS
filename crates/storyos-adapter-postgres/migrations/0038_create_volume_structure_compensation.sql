...
ALTER TABLE storyos.authoritative_commits
  DROP CONSTRAINT authoritative_commits_binding_shape;
ALTER TABLE storyos.authoritative_commits
  ADD CONSTRAINT authoritative_commits_binding_shape CHECK ((
    ...
    OR (prior_manuscript_tree_revision IS NOT NULL
      ...
      AND (
        resulting_manuscript_tree_revision = prior_manuscript_tree_revision + 1
        OR resulting_manuscript_tree_revision = prior_manuscript_tree_revision - 1
      )
  ...
  ELSIF scoped_command_kind = 'undoLatestAuthorAction' THEN
    IF scoped_result_kind = 'authoritative_applied' THEN
      IF NOT (
        ...
        RAISE EXCEPTION USING
          ERRCODE = '23514',
          MESSAGE = 'undoLatestAuthorAction applied requires prose or structure compensation authority';
      END IF;
    ELSIF (activity_count, action_count, commit_count, revision_envelope_count,
           payload_count, archival_count)
            <> (0, 0, 0, 0, 0, 0) THEN
