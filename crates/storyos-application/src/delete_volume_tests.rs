...
use super::*;
use crate::{Project, ProjectId, UserId, VolumeId};

struct Store(Mutex<usize>);

impl DeleteVolumeStore for Store {
...
fn command() -> DeleteVolumeCommand {
    ...
    let canonical_command_bytes =
        br#"{"command_schema":"storyos.command.delete-volume.request.v1"}"#.to_vec();
    let digest = {
    ...
    DeleteVolumeCommand {
        ...
        challenge_binding: ProjectCommandChallengeBinding {
            ...
            command_schema: "storyos.command.delete-volume.request.v1".to_owned(),
...
#[tokio::test]
async fn an_admitted_delete_volume_reaches_the_store() {
    let store = Store(Mutex::new(0));
    let settlement = delete_volume(&store, &command()).await.unwrap();
...
#[tokio::test]
async fn a_mismatched_delete_volume_challenge_is_a_binding_conflict() {
    let store = Store(Mutex::new(0));
    let mut mismatched = command();
    mismatched.challenge_binding.command_kind = "updateVolume".to_owned();
    assert!(matches!(
        delete_volume(&store, &mismatched).await,
