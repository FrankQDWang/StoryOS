...
impl UpdateVolumeStore for Store {
    async fn update_volume(
        ...
        *self.0.lock().unwrap() += 1;
        Ok(UpdateVolumeSettlement {
            ...
            effect: UpdateVolumeSettlementEffect::Applied {
...
fn command() -> UpdateVolumeCommand {
    ...
    UpdateVolumeCommand {
        ...
        challenge_binding: ProjectCommandChallengeBinding {
            ...
            canonical_command_digest: "sha256:storyos.command.updateVolume.jcs.v1:3c093ae1aeb68b76b87ac4244951825fa36a5a0fac4b37e609acde8ed9c1382a".to_owned(),
            idempotency_key: "key".to_owned(),
        },
        nonce_digest: "nonce".to_owned(),
        canonical_command_bytes:
            br#"{"expected_volume_revision":"2","order":"2","title":"Volume B"}"#.to_vec(),
...
#[tokio::test]
async fn an_exact_update_volume_binding_reaches_the_store() {
...
#[tokio::test]
async fn a_changed_update_volume_binding_is_refused_before_the_store() {
    let store = Store(Mutex::new(0));
    let mut changed = command();
    changed.challenge_binding.command_kind = "createVolume".to_owned();
    assert!(matches!(
        update_volume(&store, &changed).await,
...
async fn an_invalid_title_or_order_is_refused_before_the_store() {
    ...
    empty.title.clear();
    assert!(matches!(
        update_volume(&store, &empty).await,
        Err(UpdateVolumeError::BindingConflict)
    ));
    let mut zero_order = command();
    zero_order.order = 0;
    assert!(matches!(
        update_volume(&store, &zero_order).await,
        Err(UpdateVolumeError::BindingConflict)
    ));
    assert_eq!(*store.0.lock().unwrap(), 0);
}
