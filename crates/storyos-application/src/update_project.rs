...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct UpdateProjectSettlement {
    pub ids: AuthorCommandAdmissionIds,
    pub effect: UpdateProjectSettlementEffect,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum UpdateProjectSettlementEffect {
    Applied {
        title: String,
        revision: u64,
    },
    NoEffect {
        reason: storyos_core::UpdateProjectNoEffect,
    },
    Conflicted {
        reason: storyos_core::UpdateProjectConflict,
    },
    Refused {
        reason: storyos_core::UpdateProjectRefusal,
    },
}
...
impl std::fmt::Display for UpdateProjectError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Unavailable(_) => formatter.write_str("The Update Project store is unavailable"),
        }
    }
}
...
/// Owns one admitted Update Project and its atomic Core settlement.
pub trait UpdateProjectStore: Sync {
    fn update_project(
        &self,
        command: &UpdateProjectCommand,
    ) -> impl Future<Output = Result<UpdateProjectSettlement, UpdateProjectError>> + Send;
}

pub async fn update_project(
    store: &impl UpdateProjectStore,
    command: &UpdateProjectCommand,
    ...
    let command_digest = {
        ...
        format!("sha256:storyos.command.updateProject.jcs.v1:{value}")
    };
    if challenge.project_scope != command.project_scope
        ...
        || challenge.command_kind != "updateProject"
        || challenge.canonical_command_digest != command_digest
        || challenge.method != "PATCH"
        || challenge.route_template != "/api/v1/projects/{project_id}"
        || challenge.command_schema != "storyos.command.update-project.request.v1"
    ...
    store.update_project(command).await
}
