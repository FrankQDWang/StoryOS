#[test]
fn export_project_archive_wire_is_generated_from_one_closed_contract() {
    let operation = &crate::release1_project_export::EXPORT_PROJECT_ARCHIVE;
    ...
    let reviewed_operation = reviewed_catalog["operations"]
        ...
        .expect("reviewed catalog must contain exportProjectArchive");
    ...
    let request_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_project_export_artifacts::REQUEST_SCHEMA_PATH],
    )
    .expect("export request schema must be JSON");
    let response_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_project_export_artifacts::RESPONSE_SCHEMA_PATH],
    )
    ...
    assert!(openapi.contains("    post:\n      operationId: exportProjectArchive"));
    ...
    assert!(client.contains("export async function digestExportProjectArchive("));
    assert!(client.contains("export async function exportProjectArchive("));
    ...
    let graph_operation = graph["operations"]
        ...
        .iter()
        .find(|entry| entry["operation_id"] == operation.operation_id)
        .expect("contract graph must contain exportProjectArchive");
    for precondition in [
    ...
    let positive: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_project_export_artifacts::FIXTURE_PATHS[0]],
    )
