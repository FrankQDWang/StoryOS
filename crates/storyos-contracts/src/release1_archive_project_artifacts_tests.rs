#[test]
fn archive_project_wire_is_generated_from_one_closed_contract() {
    let operation = &crate::release1_archive_project::ARCHIVE_PROJECT;
    ...
    let reviewed_operation = reviewed_catalog["operations"]
        ...
        .expect("reviewed catalog must contain archiveProject");
    ...
    let request_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_archive_project_artifacts::REQUEST_SCHEMA_PATH],
    )
    .expect("archive request schema must be JSON");
    let response_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_archive_project_artifacts::RESPONSE_SCHEMA_PATH],
    )
    .expect("archive response schema must be JSON");
    ...
    assert!(client.contains("export async function digestArchiveProject("));
    assert!(client.contains("export async function archiveProject("));
    assert!(client.contains("method: \"PUT\""));
    assert!(client.contains("storyos.command.archiveProject.jcs.v1"));
    ...
    let graph_operation = graph["operations"]
        ...
        .iter()
        .find(|entry| entry["operation_id"] == operation.operation_id)
        .expect("contract graph must contain archiveProject");
    ...
    let positive: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_archive_project_artifacts::FIXTURE_PATHS[0]],
    )
    .expect("positive archive fixture must be JSON");
    assert_eq!(positive["effect"]["kind"], "authoritative_applied");
    assert_eq!(positive["receipt"]["command_kind"], "archiveProject");
    let invalid: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_archive_project_artifacts::FIXTURE_PATHS[1]],
    )
    .expect("invalid archive fixture must be JSON");
