#[test]
fn export_human_readable_manuscript_wire_is_generated_from_one_closed_contract() {
    let operation = &crate::release1_readable_export::EXPORT_HUMAN_READABLE_MANUSCRIPT;
    ...
    let reviewed_operation = reviewed_catalog["operations"]
        ...
        .expect("reviewed catalog must contain exportHumanReadableManuscript");
    ...
    let request_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_readable_export_artifacts::REQUEST_SCHEMA_PATH],
    )
    .expect("export request schema must be JSON");
    let response_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_readable_export_artifacts::RESPONSE_SCHEMA_PATH],
    )
    .expect("export response schema must be JSON");
    ...
    assert!(openapi.contains("    post:\n      operationId: exportHumanReadableManuscript"));
    ...
    assert!(client.contains("export async function digestExportHumanReadableManuscript("));
    assert!(client.contains("export async function exportHumanReadableManuscript("));
    ...
    let graph_operation = graph["operations"]
        ...
        .iter()
        .find(|entry| entry["operation_id"] == operation.operation_id)
        .expect("contract graph must contain exportHumanReadableManuscript");
    for precondition in [
    ...
    let positive: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_readable_export_artifacts::FIXTURE_PATHS[0]],
    )
