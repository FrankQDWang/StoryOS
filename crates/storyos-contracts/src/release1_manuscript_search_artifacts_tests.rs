#[test]
fn search_manuscript_wire_is_generated_from_one_closed_contract() {
    let operation = &crate::release1_manuscript_search::SEARCH_MANUSCRIPT;
    ...
    let reviewed_operation = reviewed_catalog["operations"]
        ...
        .expect("reviewed catalog must contain searchManuscript");
    ...
    let request_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_manuscript_search_artifacts::REQUEST_SCHEMA_PATH],
    )
    .expect("search request schema must be JSON");
    let response_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_manuscript_search_artifacts::RESPONSE_SCHEMA_PATH],
    )
    .expect("search response schema must be JSON");
    ...
    assert!(client.contains(
        "path: `/api/v1/projects/${encodeURIComponent(projectId)}/queries/manuscript-search`"
    ));
    ...
    let graph_operation = graph["operations"]
        ...
        .iter()
        .find(|entry| entry["operation_id"] == operation.operation_id)
        .expect("contract graph must contain searchManuscript");
    ...
    assert!(preconditions.contains(&serde_json::json!("bounded_search_query")));
    assert!(preconditions.contains(&serde_json::json!("projection_watermark_if_required")));
    assert!(preconditions.contains(&serde_json::json!("redaction_profile")));

    let positive: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_manuscript_search_artifacts::FIXTURE_PATHS[0]],
    )
    .expect("positive search fixture must be JSON");
    assert_eq!(positive["items"], serde_json::json!([]));
    assert_eq!(positive["completeness"], "complete");
    assert_eq!(positive["lag"], "0");
    let invalid: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_manuscript_search_artifacts::FIXTURE_PATHS[1]],
    )
    .expect("invalid search fixture must be JSON");
    assert!(invalid.get("source_snapshot").is_none());
}
