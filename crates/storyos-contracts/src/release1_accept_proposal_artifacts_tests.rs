#[test]
fn accept_proposal_wire_is_generated_from_one_closed_contract() {
    let operation = &crate::release1_accept_proposal::ACCEPT_PROPOSAL;
    ...
    let reviewed_operation = reviewed_catalog["operations"]
        ...
        .expect("reviewed catalog must contain acceptProposal");
    ...
    assert!(client.contains("export async function digestAcceptProposal("));
    assert!(client.contains("export async function acceptProposal("));
    ...
    let graph_operation = graph["operations"]
        ...
        .iter()
        .find(|entry| entry["operation_id"] == operation.operation_id)
        .expect("contract graph must contain acceptProposal");
    for precondition in [
