#[test]
fn delete_volume_wire_is_generated_from_one_closed_contract() {
    let operation = &crate::release1_delete_volume::DELETE_VOLUME;
    ...
    let reviewed_operation = reviewed_catalog["operations"]
        ...
        .expect("reviewed catalog must contain deleteVolume");
    ...
    let request_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_delete_volume_artifacts::REQUEST_SCHEMA_PATH],
    )
    .expect("delete volume request schema must be JSON");
    let response_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_delete_volume_artifacts::RESPONSE_SCHEMA_PATH],
    )
    .expect("delete volume response schema must be JSON");
    ...
    assert!(client.contains("export async function digestDeleteVolume("));
    assert!(client.contains("export async function deleteVolume("));
    ...
    let graph_operation = graph["operations"]
        ...
        .iter()
        .find(|entry| entry["operation_id"] == operation.operation_id)
        .expect("contract graph must contain deleteVolume");
    for precondition in [
        "server_derived_project_scope",
        "volume_scope_join",
        "expected_tree_revision",
        "child_chapter_policy",
    ] {
        assert!(
    ...
    let positive: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_delete_volume_artifacts::FIXTURE_PATHS[0]],
    )
    .expect("positive delete volume fixture must be JSON");
    assert_eq!(positive["effect"]["kind"], "authoritative_applied");
    assert_eq!(
        positive["effect"]["volume_id"],
        "018f0000-0000-7001-8000-000000000815"
    );
}
