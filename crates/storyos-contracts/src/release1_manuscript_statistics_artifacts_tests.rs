#[test]
fn get_statistics_wire_is_generated_from_one_closed_contract() {
    let operation = &crate::release1_manuscript_statistics::GET_STATISTICS;
    ...
    let reviewed_operation = reviewed_catalog["operations"]
        ...
        .expect("reviewed catalog must contain getStatistics");
    ...
    let request_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_manuscript_statistics_artifacts::REQUEST_SCHEMA_PATH],
    )
    .expect("statistics request schema must be JSON");
    let response_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_manuscript_statistics_artifacts::RESPONSE_SCHEMA_PATH],
    )
    .expect("statistics response schema must be JSON");
    ...
    assert_eq!(
        response_schema["properties"]["counting_profile"]["const"],
        "storyos.statistics.unicode-16.0.0.v1"
    );
    ...
    assert!(openapi.contains(
        "/api/v1/projects/{project_id}/manuscript/statistics:\n    get:\n      operationId: getStatistics"
    ));
    ...
    let graph_operation = graph["operations"]
        ...
        .iter()
        .find(|entry| entry["operation_id"] == operation.operation_id)
        .expect("contract graph must contain getStatistics");
    ...
    assert!(preconditions.contains(&serde_json::json!(
        "canonical_snapshot_or_projection_watermark"
    )));

    let positive: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_manuscript_statistics_artifacts::FIXTURE_PATHS[0]],
    )
    .expect("positive statistics fixture must be JSON");
    assert_eq!(
        positive["counting_profile"],
        "storyos.statistics.unicode-16.0.0.v1"
    );
