#[test]
fn update_chapter_wire_is_generated_from_one_closed_contract() {
    let operation = &crate::release1_update_chapter::UPDATE_CHAPTER;
    ...
    let reviewed_operation = reviewed_catalog["operations"]
        ...
        .expect("reviewed catalog must contain updateChapter");
    ...
    let request_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_update_chapter_artifacts::REQUEST_SCHEMA_PATH],
    )
    .expect("update chapter request schema must be JSON");
    let response_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_update_chapter_artifacts::RESPONSE_SCHEMA_PATH],
    )
    .expect("update chapter response schema must be JSON");
    ...
    assert!(client.contains("export async function digestUpdateChapter("));
    assert!(client.contains("export async function updateChapter("));
    assert!(client.contains("chapterId"));
    ...
    let graph_operation = graph["operations"]
        ...
        .iter()
        .find(|entry| entry["operation_id"] == operation.operation_id)
        .expect("contract graph must contain updateChapter");
    for precondition in [
    ...
    let positive: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_update_chapter_artifacts::FIXTURE_PATHS[0]],
    )
    .expect("positive update chapter fixture must be JSON");
    assert_eq!(positive["effect"]["kind"], "authoritative_applied");
    assert_eq!(positive["effect"]["title"], "Chapter B");
    assert_eq!(positive["effect"]["order"], "2");
}
