#[test]
fn get_manuscript_tree_wire_is_generated_from_one_closed_contract() {
    let operation = &crate::release1_manuscript_tree::GET_MANUSCRIPT_TREE;
    ...
    let reviewed_operation = reviewed_catalog["operations"]
        ...
        .expect("reviewed catalog must contain getManuscriptTree");
    ...
    let request_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_manuscript_tree_artifacts::REQUEST_SCHEMA_PATH],
    )
    .expect("tree request schema must be JSON");
    let response_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_manuscript_tree_artifacts::RESPONSE_SCHEMA_PATH],
    )
    .expect("tree response schema must be JSON");
    ...
    assert!(openapi.contains(
        "/api/v1/projects/{project_id}/manuscript/tree:\n    get:\n      operationId: getManuscriptTree"
    ));
    ...
    assert!(
        client
            .contains("path: `/api/v1/projects/${encodeURIComponent(projectId)}/manuscript/tree`")
    );
    ...
    let graph_operation = graph["operations"]
        ...
        .iter()
        .find(|entry| entry["operation_id"] == operation.operation_id)
        .expect("contract graph must contain getManuscriptTree");
    ...
    assert!(preconditions.contains(&serde_json::json!("canonical_snapshot")));
    assert!(preconditions.contains(&serde_json::json!("tree_scope_join")));

    let positive: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_manuscript_tree_artifacts::FIXTURE_PATHS[0]],
    )
    .expect("positive tree fixture must be JSON");
    assert_eq!(positive["volumes"], serde_json::json!([]));
    assert_eq!(positive["tree_revision"], "1");
    let invalid: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_manuscript_tree_artifacts::FIXTURE_PATHS[1]],
    )
    .expect("invalid tree fixture must be JSON");
    assert!(invalid.get("snapshot").is_none());
}
