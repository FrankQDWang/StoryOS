#[test]
fn create_chapter_wire_is_generated_from_one_closed_contract() {
    let operation = &crate::release1_create_chapter::CREATE_CHAPTER;
    ...
    let reviewed_operation = reviewed_catalog["operations"]
        ...
        .expect("reviewed catalog must contain createChapter");
    ...
    let request_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_create_chapter_artifacts::REQUEST_SCHEMA_PATH],
    )
    .expect("create chapter request schema must be JSON");
    let response_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_create_chapter_artifacts::RESPONSE_SCHEMA_PATH],
    )
    .expect("create chapter response schema must be JSON");
    ...
    assert!(client.contains("export async function digestCreateChapter("));
    assert!(client.contains("export async function createChapter("));
    ...
    let graph_operation = graph["operations"]
        ...
        .iter()
        .find(|entry| entry["operation_id"] == operation.operation_id)
        .expect("contract graph must contain createChapter");
    for precondition in [
    ...
    let positive: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_create_chapter_artifacts::FIXTURE_PATHS[0]],
    )
    .expect("positive create chapter fixture must be JSON");
    assert_eq!(positive["effect"]["kind"], "authoritative_applied");
    assert_eq!(positive["effect"]["title"], "Chapter A");
