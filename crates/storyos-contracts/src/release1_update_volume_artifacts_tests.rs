#[test]
fn update_volume_wire_is_generated_from_one_closed_contract() {
    let operation = &crate::release1_update_volume::UPDATE_VOLUME;
    ...
    let reviewed_operation = reviewed_catalog["operations"]
        ...
        .expect("reviewed catalog must contain updateVolume");
    ...
    let request_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_update_volume_artifacts::REQUEST_SCHEMA_PATH],
    )
    .expect("update volume request schema must be JSON");
    let response_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_update_volume_artifacts::RESPONSE_SCHEMA_PATH],
    )
    .expect("update volume response schema must be JSON");
    ...
    assert!(client.contains("export async function digestUpdateVolume("));
    assert!(client.contains("export async function updateVolume("));
    assert!(client.contains("volumeId"));
    ...
    let graph_operation = graph["operations"]
        ...
        .iter()
        .find(|entry| entry["operation_id"] == operation.operation_id)
        .expect("contract graph must contain updateVolume");
    for precondition in [
    ...
    let positive: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_update_volume_artifacts::FIXTURE_PATHS[0]],
    )
    .expect("positive update volume fixture must be JSON");
    assert_eq!(positive["effect"]["kind"], "authoritative_applied");
    assert_eq!(positive["effect"]["title"], "Volume B");
