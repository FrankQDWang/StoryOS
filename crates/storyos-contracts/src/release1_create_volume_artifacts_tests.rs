#[test]
fn create_volume_wire_is_generated_from_one_closed_contract() {
    let operation = &crate::release1_create_volume::CREATE_VOLUME;
    ...
    let reviewed_operation = reviewed_catalog["operations"]
        ...
        .expect("reviewed catalog must contain createVolume");
    ...
    let request_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_create_volume_artifacts::REQUEST_SCHEMA_PATH],
    )
    .expect("create volume request schema must be JSON");
    let response_schema: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_create_volume_artifacts::RESPONSE_SCHEMA_PATH],
    )
    .expect("create volume response schema must be JSON");
    ...
    assert!(client.contains("export async function digestCreateVolume("));
    assert!(client.contains("export async function createVolume("));
    ...
    let graph_operation = graph["operations"]
        ...
        .iter()
        .find(|entry| entry["operation_id"] == operation.operation_id)
        .expect("contract graph must contain createVolume");
    for precondition in [
    ...
    let positive: serde_json::Value = serde_json::from_slice(
        &generated[crate::release1_create_volume_artifacts::FIXTURE_PATHS[0]],
    )
    .expect("positive create volume fixture must be JSON");
    assert_eq!(positive["effect"]["kind"], "authoritative_applied");
    assert_eq!(positive["effect"]["title"], "Volume A");
}
